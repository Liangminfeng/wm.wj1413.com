...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $end_time   = date('Y-m-d', time() + 3600 * 24 * 30);  // 默认结束时间为1个月以后
    if(!empty($topic_id)){
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 格式化广告的有效日期 */
    $ads_arr['start_time']  = date('Y-m-d', $ads_arr['start_time']);
    $ads_arr['end_time']    = date('Y-m-d', $ads_arr['end_time']);
...
function get_adslist()
{
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
         ...
         /* 格式化日期 */
         $rows['start_date']    = date($GLOBALS['_CFG']['date_format'], $rows['start_time']);
         $rows['end_date']      = date($GLOBALS['_CFG']['date_format'], $rows['end_time']);
