...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    insert_article_resource($resource_type, $cat_id);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $res = $db->getAll("SELECT resource_type FROM " . $ecs->table("article_resource") . " WHERE cat_id=" . $cat['cat_id']);
    if (!empty($res))
    {
        $resource_type = array();
        foreach($res as $data)
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($exc->edit("cat_name = '$_POST[cat_name]', cat_desc ='$_POST[cat_desc]', keywords='$_POST[keywords]',parent_id = '$_POST[parent_id]', cat_type='$cat_type', sort_order='$_POST[sort_order]', show_in_nav = '$_POST[show_in_nav]'",  $_POST['id']))
    {
        ...
        insert_article_resource($_POST['resource_type'], $_POST['id']);
 ...
 * @return void
 */
function insert_article_resource($resource_type, $cat_id)
{
    ...
    {
        //取得之前的分类
        $resource = $GLOBALS['db']->getAll("SELECT resource_type FROM " . $GLOBALS['ecs']->table("article_resource") . " WHERE cat_id=" . $cat_id);
        if (empty($resource))
        {
            foreach($resource_type as $data)
            {
                ...
                $GLOBALS['db']->query("INSERT INTO " . $GLOBALS['ecs']->table("article_resource") . "(cat_id, resource_type) VALUES ('$cat_id', '$data')");
            }
        }
        else
        {
            ...
            {
                $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['ecs']->table("article_resource") . " WHERE cat_id=$cat_id AND resource_type " . db_create_in($delete_array));
            }
    ...
    {
        $GLOBALS['db']->query("DELETE FROM ". $GLOBALS['ecs']->table("article_resource") . " WHERE cat_id=" . $cat_id);
    }
}
?>
