...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    if ($is_add)
    {
        ...
        $goods = array(
            ...
            'warn_number'   => 1,
            'promote_start_date' => date('Y-m-d'),
            'promote_end_date'   => date('Y-m-d', local_strtotime('+1 month')),
            'goods_weight'  => 0,
            'give_integral' => -1,
            'rank_integral' => -1
        );
    ...
    else
    {
        ...
        if (empty($goods) === true)
        {
            ...
            $goods = array(
                ...
                'warn_number'   => 1,
                'promote_start_date' => date('Y-m-d'),
                'promote_end_date'   => date('Y-m-d', gmstr2tome('+1 month')),
                'goods_weight'  => 0,
                'give_integral' => -1,
                'rank_integral' => -1
            );
        ...
        $cat_ids = array();
         $sql = "SELECT rtcat_id FROM " . $ecs->table('goods') . " WHERE goods_id = '$_REQUEST[goods_id]'";
        $sql1 = "SELECT areas_id FROM " . $ecs->table('goods') . " WHERE goods_id = '$_REQUEST[goods_id]'";
        $goods['cat_ids'] = $db->getOne($sql);
        $goods1['areas_id'] = $db->getOne($sql1);
        $goods['cat_ids'] = explode(",", $goods['rtcat_id']);

        $goods['areas_enable'] = explode(",", $goods1['areas_id']);
        ...
        if (isset($GLOBALS['shop_id']) && ($GLOBALS['shop_id'] > 0))
        {
            ...
            {
                $gallery_img[$key]['img_url'] = $gallery_img["img_original"];
                $gallery_img[$key]['thumb_url'] = $gallery_img["thumb_url"];
            }
        }
        else
        {
    ...
    /* 啟動列車列表 */
    $trainTempList = $db->getAll("select name,code,train_id from ".$ecs->table("train")." where status=0 order by sort");
    $trainList = array();
    
    foreach ($trainTempList as $key => $val){
        $trainList[$val["train_id"]]="{$val["name"]}-{$val["code"]}";
    } 

    if($goods["train_id"]&&!in_array($goods_id["train_id"], array_keys($trainList))){

        $smarty->assign('trainGo',    true);
    }
    
    $smarty->assign('train_list',    $trainList);
    /* 模板赋值 */
    $smarty->assign('code',    $code);
    ...
    $goods['goods_area'] = explode(",", $goods['goods_area']);
...
/*         elseif ($_POST['goods_img_url'])
        ...
        if ($_CFG['auto_generate_gallery'])
        {
            ...
            $newname    = "album/".date("Ymd")."/".$image->random_filename() . substr(basename($img), $pos);

            $img = $image->qiniu_copy($img, $newname);
        ...
        {
            $goods_img = $original_img."?imageView2/1/w/{$_CFG['image_width']}/h/{$_CFG['image_height']}/q/100!|imageslim/";
        }
    ...
    else
    {
        ...
        if ($proc_thumb && isset($_POST['auto_thumb']) && !empty($original_img))
        {
            ...
            if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0)
            {
                $goods_thumb = $original_img."?imageView2/1/w/{$_CFG['thumb_width']}/h/{$_CFG['thumb_height']}/q/100!|imageslim/";
            }
            else
            {
    ...
    if($_POST['rtcat_id']){
        ...
        $_POST['rtcat_id'] = implode(",", $_POST['rtcat_id']);
    ...
    if($_POST['areas_id']){
        ...
        $_POST['areas_id'] = implode(",", $_POST['areas_id']);
    ...
    if($_POST['goods_area']){
        ...
        $_POST['goods_area'] = implode(",", $_POST['goods_area']);
    ...
    // $zxcat_id = isset($_POST['zxcat_id'])?implode(",", $_POST['zxcat_id']):
    ...
    $indepent_bonus = !empty($_POST['indepent_bonus']) ? floatval($_POST['indepent_bonus'] ) : 0;
    $is_promote = empty($_POST['is_promote']) ? 0 : 1;
    //$is_indepent_bonus = empty($_POST['is_indepent_bonus']) ? 0 : 1;
    ...
    $vip_give_integral = isset($_POST['vip_give_integral']) ? intval($_POST['vip_give_integral']) : '-1';
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, virtual_sales, is_promote, promote_price, indepent_bonus," .
                    "promote_start_date, promote_end_date, goods_img, specialicon,goods_area,goods_vip,origin_goods_vip,goods_pv,rtgoods_pv,vipgoods_pv,goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, vipintegral,give_integral,vip_give_integral, is_best, is_new, is_hot, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, rtcat_id,areas_id,goods_type, rank_integral, suppliers_id , train_id)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$virtual_sales', '$is_promote','$promote_price','$indepent_bonus', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img','$_POST[specialicon]','$_POST[goods_area]','$_POST[goods_vip]','$_POST[origin_goods_vip]','$_POST[goods_pv]', '$_POST[rtgoods_pv]','$_POST[vipgoods_pv]','$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$_POST[vipintegral]','$give_integral','$vip_give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . time() . "', '". time() ."', '$_POST[rtcat_id]','$_POST[areas_id]', '$goods_type', '$rank_integral', '$suppliers_id' ,'{$train_id}')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, virtual_sales, is_promote, promote_price,indepent_bonus,  " .
                    "promote_start_date, promote_end_date, goods_img,specialicon,goods_area,goods_vip,goods_pv,rtgoods_pv,vipgoods_pv, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral,vipintegral, give_integral, vip_give_integral,is_best, is_new, is_hot, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, rtcat_id,areas_id,goods_type, extension_code, rank_integral, train_id)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$virtual_sales', '$is_promote','$promote_price', '$indepent_bonus',".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$_POST[specialicon]','$_POST[goods_area]','$_POST[goods_vip]','$_POST[origin_goods_vip]','$_POST[goods_pv]','$_POST[rtgoods_pv]','$_POST[vipgoods_pv]', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]','$_POST[vipintegral]', '$give_integral', '$vip_give_integral','$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . time() . "', '". time() ."', '$_POST[rtcat_id]','$_POST[areas_id]', '$goods_type', '$code', '$rank_integral' ,'{$train_id}')";
        }
    }
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "is_vip = '{$_POST['is_vip']}', " .
                "specialicon= '{$_POST["specialicon"]}', " .
                "goods_area= '{$_POST["goods_area"]}', " .
                "origin_goods_vip= '{$_POST["origin_goods_vip"]}', " .
                "goods_vip= '{$_POST["goods_vip"]}', " .
                "goods_pv= '{$_POST["goods_pv"]}', " .
                "rtgoods_pv= '{$_POST["rtgoods_pv"]}', " .
                "vipgoods_pv= '{$_POST["vipgoods_pv"]}', " .
                "promote_price = '$promote_price', " .
                "indepent_bonus = '$indepent_bonus', " .
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
                ...
                "vipintegral = '$_POST[vipintegral]', " .
                "give_integral = '$give_integral', " .
                "vip_give_integral = '$vip_give_integral', " .
...
elseif ($_REQUEST['act'] == 'check_goods_level')
{
 

    $origin_goods_vip = intval($_REQUEST['origin_goods_vip']);
    $sql="SELECT id,name FROM ". $ecs->table('member_level')."WHERE id>'$origin_goods_vip'";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
...
elseif ($_REQUEST['act'] == 'edit_virtual_sales')
{
    ...
    if ($exc->edit("virtual_sales = '$virtual_sales', last_update=" .time(), $goods_id))
    {
        clear_cache_files();
