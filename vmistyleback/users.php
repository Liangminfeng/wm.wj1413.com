...
elseif ($_REQUEST['act'] == 'accountloglist')
{
    pushWechatTemplate($userid,$code,$pushData,$url);
    //pushTemplate('', $data, $url,888);
    /* 检查权限 */
...
elseif ($_REQUEST['act'] == 'updateparent')
{
    ...
    $username = empty($_POST['username']) ? '' : trim($_POST['username']);
    $parent_username = empty($_POST['parent_username']) ? '' : trim($_POST['parent_username']);
    $old_parent_username = empty($_POST['oldparent_username']) ? '' : trim($_POST['oldparent_username']);
    ...
    if(!empty($parent_username)){
        ...
        if(!$parent_id&&empty($parent_username)){
            sys_msg("推荐人账号错误", 1);
        }
    ...
    $sql22= "SELECT resource FROM " . $ecs->table('users') . " WHERE user_name = '$parent_username'";
    $parent_resource = $db->getOne($sql22);
    $sql221= "SELECT resource FROM " . $ecs->table('users') . " WHERE user_name = '$old_parent_username'";
    ...
    if($old_parent_username!=$parent_username){
...
elseif ($_REQUEST['act'] == 'editparent')
{
    ...
    $sql = "SELECT u.user_id,u.resource, u.sex,u.level_xp,u.status, u.vip_manage_account,u.birthday,u.ID_card,  u.rank_points, u.user_rank ,  u.user_vip , u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn,
    u.office_phone, u.home_phone, u.mobile_phone".
        " FROM " .$ecs->table('users'). " u LEFT JOIN " . $ecs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
