...
//require(BASE_PATH . 'vendor/Http.class.php');
...
elseif ($_REQUEST['act'] == 'useraccountexport'){
        $list=($_SESSION['useraccount_list']);
        ...
        //设置标题
        $rowVal = array(0=>'会员名称',1=>'操作日期', 2=>'金额', 3=>'姓名', 4=>'证件号码', 5=>'开户行', 6=>'支行',7=>'银行卡',8=>'联系电话',9=>'到款状态',10=>'操作员');
        ...
        foreach($list as $k => $v)
        {
            ...
            ->setCellValue('B'.$num, $v['add_date'])
            ->setCellValue('C'.$num, $v['surplus_amount'])
            ->setCellValue('D'.$num, $v['real_name'])
            ->setCellValue('E'.$num, " ".$v['ID_card'])
            ->setCellValue('F'.$num, $v['bank_name'])
            ->setCellValue('G'.$num, $v['bank_sub_name'])
            ->setCellValue('H'.$num, " ".$v['bank_card'])
            ->setCellValue('I'.$num, $v['autonym_mobile_phone'])
            ->setCellValue('J'.$num, $v['is_paid'])
            ->setCellValue('K'.$num, $v['admin_user'])
...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    $add_time = date("Y-m-d H:i:s",$account['add_time']);
    if($account['process_type']==1){
        ...
        if($is_paid == '1'){
            ...
            // 退回申请
                    ...
                    update_user_account($id, $amount, $admin_note, 2);

            new_log_account_change($account['user_id'], $amount,$_LANG['surplus_type_2'], ACT_DRAWING, 11);
               sys_msg("退回申请操作成功", 0, $link);
    ...
    if ($account['is_paid'] == 0)
    {
        ...
        {
            $user_account = get_user_surplus_frozen($account['user_id']);
            ...
            update_user_account($id, $amount, $admin_note, $is_paid);
             //log_account_change($_SESSION['user_id'], -$amount, 0, 0, 0, $_LANG['surplus_type_1'], ACT_DRAWING);
            //更新会员余额数量
        ...
        elseif($is_paid =='2')
        {
            ...
            //退回提现申请
             ...
             log_account_change($account['user_id'], -$amount, $amount, 0, 0, $_LANG['surplus_type_2'], ACT_DRAWING);
 ...
 * @return  int
 */
function get_user_surplus_frozen($user_id)
{
...
function update_user_account($id, $amount, $admin_note, $is_paid)
{
    ...
    // {
    // $r = json_decode(Http::doPost($postUrl,$data,5,"Content-Type: application/json",'json'),true);
...
function account_list()
{
    ...
    foreach ($list AS $key => $value)
    {
        ...
        $sql = "SELECT bank_card FROM " .$GLOBALS['ecs']->table('user_account'). " where id = '".$value['id']."'";    
         
        
        $list[$key]['bank_card'] =  $GLOBALS['db']->getOne($sql);
        $sql111 = "SELECT real_name FROM " .$GLOBALS['ecs']->table('users'). " where user_id = '".$value['user_id']."'";    
         
        
        $list[$key]['real_name'] =  $GLOBALS['db']->getOne($sql111);
    ...
    $_SESSION['useraccount_list'] = $list;
    return $arr;
}
