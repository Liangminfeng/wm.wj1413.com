...
if ($_REQUEST['act'] == 'list')
{
    ...
    $logdb = get_affiliate_ck($_GET['auid']);
...
elseif ($_REQUEST['act'] == 'separate')
{
    ...
    $row = $db->getRow("SELECT o.order_sn, o.is_separate, (o.goods_amount - o.discount) AS goods_amount,order_amount, o.user_id FROM " . $GLOBALS['ecs']->table('order_info') . " o".
        ...
        foreach($goods_list as $key=>$val){
            ...
            if($is_dependent){
                /*独立现金分成，金额等于各个商品设置的佣金之和*/
                 $goods_list1 = $db->getAll("SELECT goods_id,goods_number FROM " . $GLOBALS['ecs']->table('order_goods') .  " where order_id = $oid");
                 
                 foreach ($goods_list1 as $key1 => $value1) {
                     # code...
                   
                        $sql1 = "SELECT indepent_bonus FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id = '{$value1['goods_id']}'";
                  
                        $goodsinfo   =    $GLOBALS['db']->getOne($sql1);
                       
                        
                        $totalmoney += $value1['goods_number']*$goodsinfo;
            ...
            */
            ...
            if(empty($separate_by))
            {
                ...
                for ($i=0; $i < $num; $i++)
                {
                    ...
                    $row = $db->getRow("SELECT o.parent_id as user_id,u.user_name,u.nick_name,u.rank_points FROM " . $GLOBALS['ecs']->table('users') . " o" .
                    ...
                    else
                    {
                        ...
                        write_affiliate_log($oid, $up_uid, $row['user_name'], $setmoney, $setpoint, $separate_by);
                          ...
                          $pushData = array(
                                        ...
                                        'first' => array('value' => '您好，您的分销奖金已发放','color' => '#173177'),
                                        'keyword1' => array('value' => empty($row['nick_name'])?$row['user_name']:$row['nick_name'],'color' => '#000'),  // 订单号
                                        'keyword2' => array('value' => ($row['rank_points']>=10000)?'拓客商城VIP':'普通会员','color' => '#000'),   // 付款状态
                                        'keyword3' => array('value' => number_format($setmoney,2),'color' => '#000'),  //付款时间
                                        'keyword4' => array('value' => local_date('Y-m-d H:i', time()),'color' => '#000'),       // 商户名
                                         // 付款金额
                                        'remark' => array('value' => '奖金已结算至您的余额账户中,点击查看详细','color' => '#173177')
                                    );
                          
                         $url = 'index.php?c=user&a=account_list&u='.$up_uid;
                         pushWechatTemplate($up_uid, 'OPENTM413839164', $pushData, $url);
...
function get_affiliate_ck($auid)
{
    ...
    if(!empty($affiliate['on']))
    {
        ...
        {
            //推荐注册分成
            $sql = "SELECT o.*, a.log_id, a.user_id as suid,  u.user_name as auser, a.money, a.point, a.separate_type,u.parent_id as up FROM " . $GLOBALS['ecs']->table('order_info') . " o".
                    " LEFT JOIN".$GLOBALS['ecs']->table('users')." u ON o.user_id = u.user_id".
                    " LEFT JOIN " . $GLOBALS['ecs']->table('affiliate_log') . " a ON o.order_id = a.order_id" .
                    " WHERE o.user_id > 0 AND (u.parent_id > 0 AND o.is_separate = 0 OR o.is_separate > 0) $sqladd".
    ...
    // $sql = "SELECT o.*, u.user_name as auser,u.parent_id as up FROM " . $GLOBALS['ecs']->table('order_info') . " o".
    //             " LEFT JOIN".$GLOBALS['ecs']->table('users')." u ON o.user_id = u.user_id".
    //             " LEFT JOIN " . $GLOBALS['ecs']->table('affiliate_log') . " a ON o.order_id = a.order_id" .
    //             " WHERE u.parent_id = 2398";
    $sql = "SELECT o.*, a.log_id,a.user_id as suid, u.user_name as auser, a.money, a.point, a.separate_type,u.parent_id as up FROM " . $GLOBALS['ecs']->table('order_info') . " o".
   ...
   // var_dump($rt);exit;
    ...
    {
        $rrr = order_goods_train($rt['order_id']);
        ...
        if(empty($rt["is_separate"])){
            $dills = separate_dill($rt['order_id']);
            $rtinfo = "";
            foreach ($dills as $key=>$dill){
                $rtinfo .= " | ".$rt['info'].sprintf($GLOBALS['_LANG']['separate_will'], $dill['user_id'], $dill['user_name'], $dill['money'], $dill['point']);
            }
            $rt['info'] = $rtinfo;
         }
...
//判断订单里面的产品是否是列车商品
function order_goods_train($order_id){

     $sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('order_goods') . " WHERE order_id = '$order_id'";
     $order_goods_id =    $GLOBALS['db']->getOne($sql);
     $sql = "SELECT train_id FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id = '$order_goods_id'";
     $train_id =    $GLOBALS['db']->getOne($sql);
     return $train_id;
...
function separate_dill($oid){
    ...
    if (empty($row['is_separate']))
    {
        ...
        $goods_list =  $GLOBALS['db']->getAll("SELECT goods_id,goods_number FROM " . $GLOBALS['ecs']->table('order_goods') .  " where order_id = $oid");
