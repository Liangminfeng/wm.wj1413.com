...
if ($_REQUEST['act'] == 'add')
{
    ...
    $bonus_arr['send_start_date']   = date('Y-m-d');
    $bonus_arr['use_start_date']    = date('Y-m-d');
    $bonus_arr['send_end_date']     = date('Y-m-d', $next_month);
    $bonus_arr['use_end_date']      = date('Y-m-d', $next_month);
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $bonus_arr['send_start_date']   = date('Y-m-d', $bonus_arr['send_start_date']);
    $bonus_arr['send_end_date']     = date('Y-m-d', $bonus_arr['send_end_date']);
    $bonus_arr['use_start_date']    = date('Y-m-d', $bonus_arr['use_start_date']);
    $bonus_arr['use_end_date']      = date('Y-m-d', $bonus_arr['use_end_date']);
...
if ($_REQUEST['act'] == 'gen_excel')
{
    ...
    while ($val = $db->fetchRow($res))
    {
        ...
        echo date('Y-m-d', $val['use_end_date']);
        echo "\t\n";
    }
}
...
function get_bonus_list()
{
    ...
    foreach ($row AS $key => $val)
    {
        $row[$key]['used_time'] = $val['used_time'] == 0 ?
            $GLOBALS['_LANG']['no_use'] : date($GLOBALS['_CFG']['date_format'], $val['used_time']);
