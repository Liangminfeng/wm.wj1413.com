...
if ($_REQUEST['act']=='reply')
{
    ...
    $comment_info['add_time'] = date($_CFG['time_format'], $comment_info['add_time']);
    ...
    else
    {
        ...
        $reply_info['add_time'] = date($_CFG['time_format'], $reply_info['add_time']);
    }
    /* 获取管理员的用户名和Email地址 */
...
function get_comment_list()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $userinfo = $GLOBALS['db']->getOne($sql);

             $row['user_name'] = $userinfo;
        }
        /* 标记是否回复过 */
...
//            $GLOBALS['_LANG']['yes_reply'] : $GLOBALS['_LANG']['no_reply'];
        $row['content'] =mb_substr($row['content'], 0, 30);
