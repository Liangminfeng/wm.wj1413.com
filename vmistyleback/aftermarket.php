...
elseif ($_REQUEST['act'] == 'aftermarket_info') {
    ...
    $order['sign_time'] = date($_CFG['time_format'], $order['sign_time']);
    ...
    if (isset($_POST['confirm'])) {
        ...
        $operation = 'refund';
         $pushData = array(
                                        ...
                                        'keyword1' => array('value' => $return['should_return'],'color' => '#000'),  // 订单号
                                        'keyword2' => array('value' =>$return['return_cause'],'color' => '#000'),   // 付款状态
                                        'keyword3' => array('value' => local_date('Y-m-d H:i:s', time()),'color' => '#000'),  //付款时间
                                        'keyword4' => array('value' => '原路退回','color' => '#000'),       // 商户名
                                         // 付款金额
                                        'remark' => array('value' => '请注意查收','color' => '#173177')
                                    );
                          $url ='index.php?c=user&a=aftermarket_detail&ret_id='.$ret_id.'&u='.$return['user_id'];
                   
        pushWechatTemplate($return['user_id'], 'OPENTM415964302', $pushData, $url);
...
elseif ($_REQUEST['act'] == 'batch_operate_post')
{
    ...
    else
    {
        ...
        foreach ($row AS $key => $value) {
            ...
            $row[$key]['add_time'] = date('m-d H:i', $value['add_time']);
...
function operable_list($order) {
    ...
    //var_dump($_SESSION['action_list']);exit;
    if ($actions == 'all') {
