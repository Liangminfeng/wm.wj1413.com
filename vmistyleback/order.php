...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $order['pay_time']      = $order['pay_time'] > 0 ?
        date("Y-m-d H:i:s", $order['pay_time']) : $_LANG['ps'][PS_UNPAYED];

    $order['shipping_time'] = $order['shipping_time'] > 0 ?
        date("Y-m-d H:i:s", $order['shipping_time']) : $_LANG['ss'][SS_UNSHIPPED];
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
        ...
        if ($order['user_id'] > 0)
        {
            ...
            /* 计算并发放积分 */
            // if($user['rank_points']>10000){
            //      $integral = vip_integral_to_give($order);
            // }else{
            ...
            //      new_log_account_change($order['user_id'], intval($integral['custom_points']),sprintf($_LANG['order_gift_integral'], $order['order_sn']),ACT_KD,6);
            // }else{
            //      $integral = integral_to_give($order);
            // //log_account_change($order['user_id'], 0, 0, intval($integral['rank_points']), intval($integral['custom_points']), sprintf($_LANG['order_gift_integral'], $order['order_sn']));
    ...
    if($order['user_id'] > 0){
        $pushData = array(
            'first' => array('value' => '您的订单已发货 ','color' => '#173177'), //提示
            ...
            'keyword4' => array('value' => $order['address']),  // 收货信息
            'remark' => array('value' => '请注意查收！','color' => '#173177')
        );

        $code = 'OPENTM414102503';
        $order_url = 'index.php?c=user&a=order_detail&order_id='.$order_id;
        //以json格式传输
        pushWechatTemplate($order["user_id"], $code, $pushData, $order_url);
    }
...
elseif ($_REQUEST['act'] == 'delivery_cancel_ship')
{
    ...
    if ($order['order_status'] == SS_SHIPPED_ING)
    {
        ...
        if ($order['user_id'] > 0)
        {
            ...
            // }
             $integral = integral_to_give($order);
            if($integral['custom_points']){
                log_account_change($order['user_id'], 0, 0, 0, (-1) * intval($integral['custom_points']), sprintf($_LANG['return_order_gift_integral'], $order['order_sn']));
            }
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('money' == $step)
    {
        ...
        $order['surplus']       = 0;
        //$order['integral']      = 0;
        $order['integral']=floatval($_POST['integral']) >= 0 ? floatval($_POST['integral']) : 0;
        ...
        if ($order['order_amount'] > 0)
        {
            if ($old_order['user_id'] > 0)
            {
                ...
                if ($order['order_amount'] > 0)
                {
                    ...
                    {
                    /* 如果设置了积分，再使用积分支付 */
                       if (isset($_POST['integral']) && floatval($_POST['integral']) > 0)
                         {
                           /* 检查积分是否足够 */
                           $order['integral']          = $_POST['integral'];
                           $order['integral_money']    = value_of_integral(floatval($_POST['integral']));
        ...
        if ($old_order['user_id'] > 0)
        {
            ...
            if ($user_money_change != 0)
            {
                ...
                // new_log_account_change($user['user_id'], $user_money_change,sprintf($_LANG['change_use_surplus'], $old_order['order_sn']),ACT_KD, 6); 
            }
...
/* 导出订单功能开始 */
    elseif (isset($_POST['export']))
    {
        ...
        $PHPExcel->getActiveSheet()->getColumnDimension('U')->setWidth(15);//商品实付金额
        $PHPExcel->getActiveSheet()->getColumnDimension('Y')->setWidth(15);//福豆抵扣金额
        //表格标题
        ...
        $PHPExcel->getActiveSheet()->setCellValue('U3', '商品实付金额');
        $PHPExcel->getActiveSheet()->setCellValue('Y3', '鱼宝抵扣金额');
        $hang = 4;
        foreach ($order_sn_list as $order_sn) {
            /* 取得订单信息 */
            $order = order_info(0, $order_sn);
            ...
            $PHPExcel->getActiveSheet()->setCellValue('U' . ($hang), $order['formated_money_paid']);
            $PHPExcel->getActiveSheet()->setCellValue('Y' . ($hang), $order['integral_money']);
            $hang = $hang + $shuliang;
        ...
        $Writer = PHPExcel_IOFactory::createWriter($PHPExcel, 'HTML');
        ...
        foreach($order_id_list as $id_order)
        {
            ...
            if($order)
            {
                ...
                if ($_CFG['send_confirm_email'] == '1')
                {
                    ...
                    $smarty->assign('order', $order);
                    $smarty->assign('shop_name', $_CFG['shop_name']);
                    $smarty->assign('send_date', date("Y-m-d H:i:s"));
                    $smarty->assign('sent_date', date("Y-m-d H:i:s"));
                    $content = $smarty->fetch('str:' . $tpl['template_content']);
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('unship' == $operation)
    {
        ...
        if ($order['user_id'] > 0)
        {
            ...
            // }
             $integral = integral_to_give($order);
             // new_log_account_change($order['user_id'], (-1) * intval($integral['custom_points']),sprintf($_LANG['return_order_gift_integral'], $order['order_sn']), $change_type = ACT_OTHER, $account_type = 1);
             // log_account_change($order['user_id'], 0, 0, 0, (-1) * intval($integral['custom_points']), sprintf($_LANG['return_order_gift_integral'], $order['order_sn']));
    ...
    elseif ('receive' == $operation)
    {
        ...
        update_order($order_id, $arr);
         ...
         if($order_info['order_type']==9){
                 ...
                 $r1 = givePayPoints($order['user_id'],$cart_goods[0]['goods_id']);

                        if($r1){

                               new_log_account_change($order['user_id'], $r1*$cart_goods[0]['goods_number'],'订单'.$order_info['order_sn']."赠送的鱼宝",ACT_KD, 6); 
...
elseif ($_REQUEST['act'] == 'get_goods_info')
{
    ...
    //填入表头主标题
    $PHPExcel->getActiveSheet()->setCellValue('A1', '1111111111');
    //填入表头副标题
...
function return_user_surplus_integral_bonus($order)
{
   ...
   /*  if ($order['user_id'] > 0 && $order['surplus'] > 0)
    ...
    if ($order['user_id'] > 0 && $order['integral'] > 0)
    {
        ...
        new_log_account_change($order['user_id'], $order['integral'],sprintf($GLOBALS['_LANG']['return_order_integral'], $order['order_sn']), ACT_OTHER, 6);
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $where = 'WHERE order_status != '.OS_HIDE;
        if ($filter['order_sn'])
        {
...
function delivery_order_info($delivery_id, $delivery_sn = '')
{
    ...
    if ($delivery)
    {
        ...
        $delivery['formated_update_time']    = date("Y-m-d H:i:s", $delivery['update_time']);
...
function back_order_info($back_id)
{
    ...
    if ($back)
    {
        ...
        $back['formated_update_time']    = date("Y-m-d H:i:s", $back['update_time']);
        $back['formated_return_time']    = date("Y-m-d H:i:s", $back['return_time']);
...
function get_site_root_url()
{
    return 'http://' . $_SERVER['HTTP_HOST'] . str_replace('/' . ADMIN_PATH . '/order.php', '', PHP_SELF);
       ...
       /*7天后自动收货*/
function contraborderautoreceive()
{
        /**/
      ...
      $orders = $GLOBALS['db']->query("SELECT user_id,order_id,order_sn,order_status,pay_status,shipping_time FROM " . $GLOBALS['ecs']->table("order_info") . " WHERE order_status=5 and pay_status = 2 and shipping_status = 1 ");
        ...
        foreach ($orders as $key => $value) {
              ...
              $now_time = time();
              $date=floor(($now_time-$value['shipping_time'])/86400);
                 $order_info = order_info($value['order_id']);
              ...
              if($date>15){
                ...
                $arr['confirm_receive_time'] = time();
                 ...
                 $r = update_order($value['order_id'], $arr);
                 if($order_info['order_type']==9){

                         $cart_goods = order_goods($value['order_id']);
                ...
                order_action($value['order_sn'], $value['order_status'], SS_RECEIVED, $value['pay_status'], "");
