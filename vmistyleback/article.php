...
if ($_REQUEST['act'] == 'insert')
{
    ...
    if ((isset($_FILES['file2']['error']) && $_FILES['file2']['error'] == 0) || (!isset($_FILES['file2']['error']) && isset($_FILES['file2']['tmp_name']) && $_FILES['file2']['tmp_name'] != 'none'))
    {
        ...
        $file_url2 = $image->upload_image($_FILES['file2'], 'article');
        // 复制文件
        /* $res = upload_article_file($_FILES['file']);
         if ($res != false)
         {
...
if ($_REQUEST['act'] =='update')
{
    ...
    /* 取得文件地址 */
    $file_url2 = '';
    if (empty($_FILES['file2']['error']) || (!isset($_FILES['file2']['error']) && isset($_FILES['file2']['tmp_name']) && $_FILES['file']['tmp_name'] != 'none'))
    {
        // 检查文件格式
        if (!check_file_type($_FILES['file2']['tmp_name'], $_FILES['file2']['name'], $allow_file_types))
        {
        ...
        // 复制文件
        $file_url2 = $image->upload_image($_FILES['file2'], 'aritilce');
    }
