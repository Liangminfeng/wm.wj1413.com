...
$exc = new exchange($ecs->table("poster_cat"), $db, 'cat_id', 'cat_name');
/* act操作项的初始化 */
...
{
    admin_priv('postercat_list');
    $postercat = poster_cat_list(0, 0, false);

    foreach ($postercat as $key => $cat)
    {
        $postercat[$key]['type_name'] = $_LANG['type_name'][$cat['cat_type']];
    }
    $smarty->assign('ur_here',     $_LANG['32_postercat_list']);
    $smarty->assign('action_link', array('text' => $_LANG['postercat_add'], 'href' => 'postercat.php?act=add'));
    $smarty->assign('full_page',   1);
    $smarty->assign('postercat',        $postercat);

    assign_query_info();
    $smarty->display('postercat_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('postercat_list.htm'));
}
...
elseif ($_REQUEST['act'] == 'add')
{
   ...
   // admin_priv('article_cat');
    ...
    $smarty->assign('action_link', array('text' => $_LANG['02_articlecat_list'], 'href' => 'postercat.php?act=list'));
    $smarty->assign('form_action', 'insert');

    assign_query_info();
    $smarty->display('postercat_info.htm');
}
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($_POST['parent_id'] > 0)
    {
        ...
        $sql = "SELECT cat_type FROM " . $ecs->table('poster_cat') . " WHERE cat_id = '$_POST[parent_id]'";
    ...
    $resource_type = !empty($_POST['resource_type'])  ? $_POST['resource_type'] : array();
    $sql = "INSERT INTO ".$ecs->table('poster_cat')."(cat_name, cat_type, cat_desc,keywords, parent_id, sort_order, show_in_nav)
           VALUES ('$_POST[cat_name]', '$cat_type',  '$_POST[cat_desc]','$_POST[keywords]', '$_POST[parent_id]', '$_POST[sort_order]', '$_POST[show_in_nav]')";
    $db->query($sql);
    $cat_id =  $db->insert_id();

    insert_poster_resource($resource_type, $cat_id);
    if($_POST['show_in_nav'] == 1)
    {
        $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $ecs->table('nav') . " WHERE type = 'middle'");
        $vieworder += 2;
        //显示在自定义导航栏中
        $sql = "INSERT INTO " . $ecs->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $_POST['cat_name'] . "', 'a', '" . $db->insert_id() . "','1','$vieworder','0', '" . build_uri('poster_cat', array('acid'=> $db->insert_id()), $_POST['cat_name']) . "','middle')";
        $db->query($sql);
    }

    admin_log($_POST['cat_name'],'add','postercat');

    $link[0]['text'] = $_LANG['continue_add'];
    $link[0]['href'] = 'postercat.php?act=add';

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'postercat.php?act=list';
    clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $options    =   poster_cat_list(0, $cat['parent_id'], false);
    $select     =   '';
    ...
    $res = $db->getAll("SELECT resource_type FROM " . $ecs->table("poster_resource") . " WHERE cat_id=" . $cat['cat_id']);
    if (!empty($res))
    {
        $resource_type = array();
        foreach($res as $data)
        {
            $resource_type[$data['resource_type']] = 1;
        }
    ...
    $smarty->assign('ur_here',     $_LANG['postercat_edit']);
    $smarty->assign('action_link', array('text' => $_LANG['02_postercat_list'], 'href' => 'postercat.php?act=list'));
...
elseif ($_REQUEST['act'] == 'update')
{
   ...
   // admin_priv('poster_cat');
    ...
    $row = $db->getRow("SELECT cat_type, parent_id FROM " . $ecs->table('poster_cat') . " WHERE cat_id='$_POST[id]'");
    ...
    /* 检查设定的分类的父分类是否合法 */
    $child_cat = poster_cat_list($_POST['id'], 0, false);
    if (!empty($child_cat))
    {
    ...
    $dat = $db->GetRow("SELECT cat_name, show_in_nav FROM ". $ecs->table('poster_cat') . " WHERE cat_id = '" . $_POST['id'] . "'");
    if ($exc->edit("cat_name = '$_POST[cat_name]', cat_desc ='$_POST[cat_desc]', keywords='$_POST[keywords]',parent_id = '$_POST[parent_id]', cat_type='$cat_type', sort_order='$_POST[sort_order]', show_in_nav = '$_POST[show_in_nav]'",  $_POST['id']))
    {
        ...
        if($_POST['show_in_nav'] != $dat['show_in_nav'])
        {
            if($_POST['show_in_nav'] == 1)
            {
                ...
                if(empty($nid))
                {
                    $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $ecs->table('nav') . " WHERE type = 'middle'");
                    $vieworder += 2;
                    $uri = build_uri('poster_cat', array('acid'=> $_POST['id']), $_POST['cat_name']);
                    //不存在
        ...
        insert_poster_resource($_POST['resource_type'], $_POST['id']);
        $link[0]['text'] = $_LANG['back_list'];
        $link[0]['href'] = 'postercat.php?act=list';
        $note = sprintf($_LANG['catedit_succed'], $_POST['cat_name']);
        admin_log($_POST['cat_name'], 'edit', 'postercat');
        clear_cache_files();
        sys_msg($note, 0, $link);
...
elseif ($_REQUEST['act'] == 'remove')
{
   ...
   // check_authz_json('article_cat');
    ...
    $sql = "SELECT cat_type FROM " . $ecs->table('poster_cat') . " WHERE cat_id = '$id'";
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('poster_cat') . " WHERE parent_id = '$id'";
    if ($db->getOne($sql) > 0)
    {
        /* 还有子分类，不能删除 */
    ...
    /* 非空的分类不允许删除 */
    $sql = "SELECT COUNT(*) FROM ".$ecs->table('poster')." WHERE cat_id = '$id'";
    if ($db->getOne($sql) > 0)
    {
    ...
    $url = 'postercat.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
 ...
 * @return void
 */
function insert_poster_resource($resource_type, $cat_id)
{
    ...
    {
        //取得之前的分类
        $resource = $GLOBALS['db']->getAll("SELECT resource_type FROM " . $GLOBALS['ecs']->table("poster_resource") . " WHERE cat_id=" . $cat_id);
        if (empty($resource))
        {
            foreach($resource_type as $data)
            {
                ...
                $GLOBALS['db']->query("INSERT INTO " . $GLOBALS['ecs']->table("poster_resource") . "(cat_id, resource_type) VALUES ('$cat_id', '$data')");
            }
        }
        else
        {
            ...
            {
                $old_data[] = $data['resource_type'];
            }
            $delete_array = array_diff($old_data, $resource_type);
          
            if (!empty($delete_array))
            {
                $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['ecs']->table("poster_resource") . " WHERE cat_id=$cat_id AND resource_type " . db_create_in($delete_array));
            }
            $insert_array = array_diff($resource_type, $old_data);
    ...
    {
        $GLOBALS['db']->query("DELETE FROM ". $GLOBALS['ecs']->table("poster_resource") . " WHERE cat_id=" . $cat_id);
    }
}
?>
