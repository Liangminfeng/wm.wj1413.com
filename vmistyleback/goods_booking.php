...
if ($_REQUEST['act'] =='update')
{
    ...
    $sql = "UPDATE  ".$ecs->table('booking_goods').
            " SET is_dispose='1', dispose_note='$dispose_note', ".
                    "dispose_time='" .time(). "', dispose_user='".$_SESSION['admin_name']."'".
...
function get_bookinglist()
{
    ...
    {
        $row[$key]['booking_time'] = date($GLOBALS['_CFG']['time_format'], $val['booking_time']);
    }
...
function get_booking_info($id)
{
    ...
    /* 格式化时间 */
    $res['booking_time'] = date($_CFG['time_format'],$res['booking_time']);
    if (!empty($res['dispose_time']))
    {
        $res['dispose_time'] = date($_CFG['time_format'],$res['dispose_time']);
    }
