...
elseif ($_REQUEST['act'] == 'allot')
{
    ...
    $r =  admin_priv('allot_priv');

    if ($_SESSION['admin_id'] == $_GET['id']||$_SESSION['admin_id']!==1)
    {
...
elseif ($_REQUEST['act'] == 'update_allot')
{
    ...
    // if(isset($_POST['checkall'])&&$_POST['checkall']=='checkbox'){
    //     $sql = "UPDATE " .$ecs->table('admin_user'). " SET action_list = 'all', role_id = '' ".
...
function get_admin_userlist()
{
    ...
    {
        $list[$key]['add_time']     = date($GLOBALS['_CFG']['time_format'], $val['add_time']);
        $list[$key]['last_login']   = date($GLOBALS['_CFG']['time_format'], $val['last_login']);
    }
