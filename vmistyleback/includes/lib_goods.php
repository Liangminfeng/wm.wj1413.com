    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            if (!$proc_thumb)
            {
                ...
                $thumb_url = $img_original."?imageView2/1/w/{$GLOBALS['_CFG']['thumb_width']}/h/{$GLOBALS['_CFG']['thumb_height']}/q/100!|imageslim/";
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT train_id,goods_id, goods_name, goods_type, goods_sn, virtual_sales, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
