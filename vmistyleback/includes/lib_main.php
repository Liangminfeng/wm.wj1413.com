 ...
 * @return  void
 */
function admin_log($sn = '', $action, $content ,$type = "system")
{
    global $db;
    if($type=="system"){
...
function post_log($data,$postUrl,$timeout=5,$user_id)
{

       require_once ROOT_PATH . '/include/vendor/Http.class.php'; 
    ...
    # code...
      ...
      do {
          ...
          $req = $ol->doPost($postUrl,$data,5,"Content-Type: application/json",'json');
...
function api_log($data,$return,$user_id)
{
    global $ecs, $db, $_LANG;
    ...
    $sql = 'INSERT INTO ecs_api_log  (log_time, user_id, log_info,ip_address,posturl,log_return) ' .
            "VALUES('$log_time', '$user_id', '$log_info','$ip_address','$posturl','$log_return')";
...
function get_bonus_type()
{
     ...
     * @return  void
     */
    function log_bonus_change1($user_id, $bonus_money = 0, $change_desc,$change_type = ACT_OTHER) {
        /* 插入帐户变动记录 */
        $account_log = array(
            'user_id' => $user_id,
            'user_money' => $bonus_money,
        ...
        $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('account_log') . ' (user_id, bonus_money, change_time, change_desc,change_type) ' .
            " VALUES ('$user_id', '$bonus_money', '" . time() . "', '$change_desc','$change_type')";
...
function get_user_vip_list()
{
    ...
    $user_vip_list[1] = "店主";
    $user_vip_list[2] = "服务商";
    $user_vip_list[3] = "社区合伙人";
    $user_vip_list[4] = "拓客合伙人";
 ...
 * @return      array
 */
function talk_list_info()
{

    /* 查询 */
    $sql = "SELECT talk_id, title
            FROM " . $GLOBALS['ecs']->table("talk") . "
            where is_show=1";
...
{
    /* 查询 */
    $talk_list = talk_list_info();

    /* 供货商名字 */
    $talk_list_name = array();
    if (count($talk_list) > 0)
    {
        foreach ($talk_list as $talk)
        {
            $talk_list_name[$talk['talk_id']] = $talk['title'];
        }
    }

    return $talk_list_name;
}
/**
 * 供货商名
 *
...
function pushTemplate($code = '', $data = array(), $url = '',$uid = '')
{
    ...
    $sql = 'SELECT openid FROM ' . $ecs->table('users') . " WHERE user_id = '$uid'";

    $openid = $db->getOne($sql);//o3-571D5mLIlFs6bb_MBnQz_HXd8
    $sql = 'SELECT openid FROM ' . $ecs->table('users') . " WHERE user_id = '$uid'";

    $openid = $db->getOne($sql);
    $template = M()->table('wechat_template')->field('title, content')->where(array('code'=>$code, 'status'=>1))->find();
    $template = $db->getOne('SELECT title, content FROM ' . $ecs->table('wechat_template') . " WHERE code = '$code' and status=1");
