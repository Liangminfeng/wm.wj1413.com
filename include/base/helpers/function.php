 ...
 * @return  array   $data
 */
function read_data_cache($cache_name) {
    if (defined("DEBUG_MODE")&&(DEBUG_MODE & 2) == 2) {
        return false;
    }
    static $result = array();
    ...
    $cache_file_path = ROOT_PATH . 'data/caches/data_caches/' . $cache_name . '.php';
    if (file_exists($cache_file_path)) {
        include_once($cache_file_path);
        $result[$cache_name] = $data;
        return $result[$cache_name];
 ...
 * @return
 */
function write_data_cache($cache_name, $caches) {
    if (defined("DEBUG_MODE")&&(DEBUG_MODE & 2) == 2) {
        return false;
    }
    //增加目录状态判断 by ecmoban carson
    $static_caches = ROOT_PATH . 'data/caches/data_caches/';
    if (!is_dir($static_caches)) {
        @mkdir($static_caches, 0777);
    }
    $cache_file_path = $static_caches . $cache_name . '.php';
