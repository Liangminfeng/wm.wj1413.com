...
class Controller {
    ...
    protected function pageInit($request,$url=''){
        $url = str_replace(urlencode('{page}'), '{page}', $url);
        $page = is_object($this->pager ['obj']) ? $this->pager ['obj'] : new Page ();
        $cur_page = $page->getCurPage($url);
        $pageSize = $_REQUEST["page_size"]?$_REQUEST["page_size"]:10;
        $limit_start = ($cur_page - 1) * $pageSize;
        ...
        //$this->assign("pager", ["page"=>$cur_page,"page_size"=>$pageSize]);
        return $this->pager;
    }
    // 分页结果显示
