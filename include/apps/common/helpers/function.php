...
function pushTemplate($code = '', $data = array(), $url = '',$uid = ''){
    ...
    $openid = '';
    if ($user_id) {
        $openid = M()->table('users')->field('openid')->where(array('user_id'=> $user_id))->getOne();
    ...
    if ($openid && $template['title']) {
        ...
        $data['remark'] = !empty($template['content']) ? array('value' => $template['content'],'color' => '#173177') : $data['remark'];
...
function post_log($data,$postUrl,$timeout=5)
{
    ...
    do {
          ...
          $req = Http::doPost($postUrl,$data,5,"Content-Type: application/json",'json');
...
function object_array($array){
  ...
  return $array;
    ...
    /*访问拓客接口获得数据*/
    function getTianMeiShi($mname,$data){
        $username = "TenmaxScore";
        $password = "123456";
        $keymd5 = "tmx123789";
        $md5password = md5($password);

        $jsonstring = hmac(json_encode($data),$keymd5);
        $newad = array();
          $newad[] = (array)json_encode($data,true);
        
        $posturl = "http://123.1.150.125:7799/TmxOrgtest/API.ashx?mname=".$mname."&username=".$username."&password=".$md5password."&data=".json_encode($data,JSON_UNESCAPED_SLASHES)."&signValue=".$jsonstring;
       
        $req = Http::doGet($posturl,5,"Content-Type: application/json");
        $result = json_decode($req);
        if($result->resultCode=='000001'){
...
/*sign签名算法
$s1 = 为商户秘钥secretKey
$s2 = 将所有传递的参数根据key正序排序，并组合成key1=value1&key2=value2…..这样的字符串，
sign = md5($s1.”&”.$2);*/
function signfun($signarray){

    $s1 = SECRET_KEY;
    $s2 = '';
    $array = ksort($signarray);
    ...
    $s2 = stripslashes(rtrim($s2,"&"));

    return md5($s1."&".$s2);
}
function apiverify($postdata){
     $validarray = $postdata;     
                unset($validarray['sign']);
                if($postdata['sign']!==signfun($validarray))
                {
...
function encryptapitoken(){
    ...
    $result = md5(API_TOKEN_CODE1 . API_TOKEN_CODE2.date('Y-m-d H'));

    return $result;

}
function tripzero($money){
     $ifzero = substr($money,-1,2);
                            if($ifzero=='00'){
                                        $r = floor($money);
                            }else{
                                        $r = $money;
...
function sendTemplate($openid, $code = '' ){
    ...
    if($list){
        foreach($list as $k=>$v){
            ...
            M()->table('wechat_template_log')->data(array('msgid' => $rs['msgid'],'status' => 1))->where(array('code' => $v['code'], 'openid' => $v['openid']))->update();
            return true;
        }
    }
    return false;
...
function log_write($arg, $file = '', $line = '') {
    if (defined("DEBUG_MODE")&&(DEBUG_MODE & 4) != 4) {
        return;
    }
...
function is_mobile($user_mobile) {
    $chars = '/^(1(([35][0-9])|(47)|[8][0126789]))\d{5,15}$/';
 ...
 * 格式化日期
 * @param string $format
 * @param string|int date or timpstamp
 * @return string
 */
...
/*下载远程图片*/
function downloadwechatavatarimg($url, $path = 'upload/') {

$url = "http://thirdwx.qlogo.cn/mmopen/vi_32/DYAIOgq83eqaPEKZvicAlBibVY7YodzXZIZjaWr9zBKI1UpTZsvniaiaibBdSwjx8WaRpvnrxGiaJpLYo2hia01LlHeyQ/132";
...
$filename = $path . date("YmdHis") . uniqid() . $exf;//这里默认是当前文件夹，可以加路径的 可以改为$filepath = '../'.$filename
$res = file_put_contents($filename, $content);//同样这里就可以改为$res = file_put_contents($filepath, $content);
}
}
function put_file_from_url_content($url, $saveName, $path = 'upload/') {
