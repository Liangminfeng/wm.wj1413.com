...
class ArticleBaseModel extends BaseModel {
     ...
     * @return array
     */
    public function get_cat_articles($cat_id, $page = 1, $size = 20, $requirement = '',$type = 'default',$resourcetype = false) {
        // 取出所有非0的文章
        if ($cat_id == '-1') {
            $cat_str = 'cat_id > 0';
        } else {
...
/*                 $arr[$article_id]['id'] = $article_id;
     ...
     * @return mix
     */
    function article_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0,$resourcetype =false) {
        ...
        //var_dump($res);exit;
        $options = $this->article_cat_options($cat_id, $res,$resourcetype); // 获得指定分类下的子分类的数组
     ...
     * @return void
     */
    function article_cat_options($spec_cat_id, $arr,$resourcetype=false) {
