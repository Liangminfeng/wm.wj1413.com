...
class BaseModel extends Model {
    ...
    public function select($condition = '', $field = '', $order = '', $limit ='', $page = null) {
        if(!empty($page)){
            if(empty($page["pageSize"]))$page["pageSize"] = 10;
            $start = ($page["pageNo"]-1)*$page["pageSize"];
            $limit = "{$start},{$page["pageSize"]}";
        } 
        return parent::select($condition, $field, $order, $limit);
    }
    /**
     * 
     * @param Object $data
    ...
    function error($message){
        return ["msg"=>$message,"status"=>false];
    }
     ...
     * @return array
     */
    function listResult($list,$total,$page){
        $data = array(
            "data" => $list,
            "total"=> $total,
            "page" => $page
