...
class ClipsBaseModel extends BaseModel {
    ...
    public function get_collection_goods($user_id, $num = 10, $start = 0) {
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.market_price,g.integral, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, " .
                'g.promote_price, g.promote_start_date,g.promote_end_date, c.rec_id, c.order_type,c.is_attention' .
        ...
        if (is_array($res)) {
            foreach ($res as $row) {
                ...
                // $goods_list[$row['goods_id']]['shop_price'] = price_format($row['shop_price']-$row['integral'])."+".number_format(($row['integral']*100/C(integral_scale)),0)."积分";
                $goods_list[$row['goods_id']]['shop_price'] =$row['shop_price'];
                $goods_list[$row['goods_id']]['order_type'] =$row['order_type'];
    ...
    public function insert_user_account($surplus, $amount) {
        ...
        $data['is_paid'] = 0;
        $data['bank_card'] = $surplus['bank_card'];
        $data['bank_no'] = $surplus['bank_card'];
        $data['bank_realname'] = $surplus['bank_realname'];
        $data['bank_name'] = $surplus['bank_name'];
        $data['bank_sub_name'] = $surplus['bank_sub_name'];

        return $this->insert($data);
    }

    public function findfrozemoney($user_id){
        $sql = "SELECT sum(amount) as total  FROM " . $this->pre . "user_account WHERE is_paid=0 and user_id = ".$user_id;
        $rows = $this->row($sql);
        return $rows['total'];
    }
    public function findwithdmoney($user_id){
        $sql = "SELECT sum(amount) as total  FROM " . $this->pre . "user_account WHERE is_paid=1 and user_id = ".$user_id;
        $rows = $this->row($sql);
        return $rows['total'];
    }
    ...
    public function get_account_log($user_id, $num, $start) {
        ...
        if (is_array($list)) {
            foreach ($list as $vo) {
                ...
                $vo['bank_card'] = ($vo['bank_card'])?$vo['bank_card']:0;
    ...
    public function get_user_surplus($user_id) {
        ...
        return $this->field('SUM(account)', $condition);
    }
     /**
     * 查询会员下单的金额总值
     * @access  public
     ...
     * @return  int
     */
    public function get_user_order_amount($user_id) {
        $this->table = 'order_info';
        $condition['user_id'] = $user_id;
        return $this->field('SUM(order_amount)', $condition);
    }
     ...
     * @return  int
     */
    public function get_user_frozen($user_id) {
        $this->table = 'account_log';
        $condition['user_id'] = $user_id;
        return $this->field('SUM(frozen_money)', $condition);
    }
    ...
    public function get_user_default($user_id) {
        ...
        $sql = "SELECT  autonym,rank_points, resource,user_avatar,user_rank,user_vip,vip_manage_account,user_name,credit_line,nick_name,parent_id, last_login,mobile_phone, is_validated FROM " . $this->pre . "users WHERE user_id = '$user_id'";
        $row = $this->row($sql);        
        $info = array();
        if($row['parent_id'] > 0){
        ...
        $info['user_avatar'] = $row['user_avatar'];
        $info['nick_name'] = getEmoji($row['nick_name']);
        $info['user_name'] = $row['user_name'];
        $info['user_rank'] = $row['user_rank'];
        $info['autonym'] = $row['autonym'];
        $info['resource'] = $row['resource'];
        $info['address'] = $row['address'];
        $info['autonym_mobile_phone'] = $row['autonym_mobile_phone'];
        $info['mobile_phone'] = $row['mobile_phone'];
        $info['formated_credit_line'] = price_format($info['credit_line'], false);
        
        if($row){
             $sql1 = "SELECT rank_name FROM " . $this->pre . "user_rank WHERE min_points <".$row['rank_points'] ." and max_points > ".$row['rank_points'];
        $rows1 = $this->row($sql1);
        $info['rank_name'] = $rows1  ? $rows1['rank_name']: "拓客商城会员";
        }
    ...
    function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER) {
        ...
        //是否創客
        if($rank_points!==0&&$_SESSION["user_id"]){
            $user =$this->row("SELECT rank_points FROM " . $this->pre . 'users ' . " WHERE user_id={$_SESSION["user_id"]}");
            $sql = "SELECT * FROM " . $this->pre . 'user_rank ' . " WHERE rank_id=2";
            $rank = $this->row($sql);
            $_SESSION["chuangke"] =  $user['rank_points']>$rank["min_points"]?true:false;
        }
        if($r){
            return true;
        }else{
    ...
    function finduseraccount($user_id,$account_name){
         $sql = "SELECT ".$account_name." FROM " . $this->pre . 'account ' . " WHERE user_id=$user_id";
         $useraccount= $this->row($sql);
       
         return $useraccount[$account_name];

    }
        function newnew_log_account_change($user_id, $amount = 0,$change_desc = '', $change_type = ACT_OTHER, $account_type = 1) {
        
        /*判断某用户的account表里面是否有记录*/
        $accountisexist =  $this->model->table('account')
                ->field('user_id')
                ->where('user_id =' . $user_id)
                ->getOne();
        if(!$accountisexist){
        ...
        $this->table = 'account_log';
        $r = $this->insert($account_log);
     
        /* 更新用户信息 
        账号类别 1:balance 2:bonus_distributor 3:bonus_vip 4:bonus_retail 5:rank_points 6:pay_points 7:vcoin  8:fish_ticket
        */
        ...
        switch ($account_type) {
            case '1':
                # balance...
                $sql = "UPDATE " . $this->pre .
                "account SET balance = balance + ('$amount')" .
             ...
             case '2':
                # bonus_distributor...
                $sql = "UPDATE " . $this->pre .
                "account SET bonus_distributor = bonus_distributor + ('$amount')" .
             ...
             case '3':
                # bonus_vip...
                $sql = "UPDATE " . $this->pre .
                "account SET bonus_vip = bonus_vip + ('$amount')" .
             ...
             case '4':
                # bonus_retail...
                $sql = "UPDATE " . $this->pre .
                "account SET bonus_retail = bonus_retail + ('$amount')" .
             ...
             case '5':
                # rank_points...
                $sql = "UPDATE " . $this->pre .
                "account SET rank_points = rank_points + ('$amount')" .
             ...
             case '6':
                # pay_points...
                $sql = "UPDATE " . $this->pre .
                "account SET pay_points = pay_points + ('$amount')" .
            ...
            case '7':
                # vcoin...
                $sql = "UPDATE " . $this->pre .
                "account SET vcoin = vcoin + ('$amount')" .
            ...
            case '8':
                # fish_ticket...
                $sql = "UPDATE " . $this->pre .
                "account SET fish_ticket = fish_ticket + ('$amount')" .
            ...
            case '9':
                # frozen_money“需要审核”..
                $sql = "UPDATE " . $this->pre .
                "account SET frozen_money = frozen_money + ('$amount')" .
            ...
            case '10':
                # 需要审核的“已提现”...
                $sql = "UPDATE " . $this->pre .
                "account SET withdraw = withdraw + ('$amount')" .
            ...
            case '11':
                # 零钱账户..
                $sql = "UPDATE " . $this->pre .
                "account SET small_change = small_change + ('$amount')" .
    ...
    /*生成account表和account_log*/
    function create_new_log_account($user_id){
        $account =  $this->model->table('vip_account')
    ...
    /*20190515新的记录函数*/
    function new_log_account_change($user_id, $amount = 0,$change_desc = '', $change_type = ACT_OTHER, $account_type = 1,$logtype = 1) {
        ...
        */
        if($logtype){
            ...
            case '12':
                ...
                "account SET lxpoint = lxpoint + ('$amount')" .
            ...
            case '13':
                ...
                "account SET train_money = train_money + ('$amount')" .
    ...
    /*$inouttype 1代表转入2转出*/
    function totalThisMonth($user_id,$change_type,$type,$inouttype){

        $beginThismonth=mktime(0,0,0,date('m'),1,date('Y'));
        $endThismonth=mktime(23,59,59,date('m'),date('t'),date('Y'));
        
            /*转入*/
            $sql = "SELECT sum(amount) as total  FROM " . $this->model->pre . "account_log WHERE user_id='".$user_id."'  and account_type = '3'  or account_type = '4' and change_time between '". $beginThismonth."' and '".$endThismonth."'";
        ...
        return abs($res['total']);
        

    }
    function newtotalThisMonth($user_id,$change_type,$type,$inouttype){

        $beginThismonth=mktime(0,0,0,date('m'),1,date('Y'));
        $endThismonth=mktime(23,59,59,date('m'),date('t'),date('Y'));
        
            /*转入*/
            $sql = "SELECT sum(amount) as total  FROM " . $this->model->pre . "account_log WHERE user_id='".$user_id."' and change_type in(19,4) and account_type in ('".$type."')  and change_time between '". $beginThismonth."' and '".$endThismonth."'";
        ...
        function sumtotalprofitlog($user_id,$change_type,$type,$inouttype){

   
        
            /*转入*/
            $sql = "SELECT count(*) as total  FROM " . $this->model->pre . "account_log WHERE user_id='".$user_id."' and change_type in(19,8,4) and account_type in (".$type.") ";
     ...
     function activitysumtotalprofitlog($user_id,$change_type,$type,$inouttype){

   
        
            /*转入*/
            $sql = "SELECT sum(amount) as total  FROM " . $this->model->pre . "account_log WHERE user_id='".$user_id."' and change_type =19 and account_type =14";
        ...
        function newtotalwithrawThisMonth($user_id,$change_type,$type,$inouttype){

        $beginThismonth=mktime(0,0,0,date('m'),1,date('Y'));
        $endThismonth=mktime(23,59,59,date('m'),date('t'),date('Y'));
        
            /*转入*/
            $sql = "SELECT sum(amount) as total  FROM " . $this->model->pre . "account_log WHERE user_id='".$user_id."' and change_type =19 and account_type in ('".$type."')  and change_time between '". $beginThismonth."' and '".$endThismonth."'";
    ...
    function  userAccount($user_id){
        ...
        $where['user_id'] = $user_id;
        $res = $this->model->table('account')->field('balance ,train_money,small_change,bonus_distributor ,bonus_vip,bonus_retail,lxpoint,cbpoint,pay_points')->where($where)->find();

        return $res;
    }
    function totaluserachievement_vip($user_id){
        $this->table = 'achievement_vip';
        $condition['user_id'] = $user_id;
        $condition['status'] = $user_id;
        return $this->field('SUM(money)', $condition);
    }
    function totaluserachievement_retail($user_id){
       
        $this->table = 'achievement_retail';
        $condition['user_id'] = $user_id;
    
        return $this->field('SUM(money)', $condition);
      
    }
    function selectBonusLog($user_id,$page,$size,$type = ''){
        
        switch ($type) {
            case '1':
                # code...待解算
                 // $sql = "SELECT * FROM " . $this->pre . "achievement_vip WHERE user_id = '$user_id' and status=0 order by created_at desc limit " . $page . ',' . $size;
                $sql = " SELECT * FROM `ecs_achievement_vip` where user_id= '$user_id' and status=0 UNION SELECT * FROM `ecs_achievement_retail` where user_id= '$user_id' and status=0 order by created_at desc limit $page, $size"; 
                break;
            case '2':
                # code...已结算
                 // $sql = "SELECT * FROM " . $this->pre . "achievement_vip WHERE user_id = '$user_id' and status=1 order by created_at desc limit " . $page . ',' . $size;
                 $sql = " SELECT * FROM `ecs_achievement_vip` where user_id= '$user_id' and status=1 UNION SELECT * FROM `ecs_achievement_retail` where user_id= '$user_id' and status=1 order by created_at desc limit $page, $size"; 
                break;
            case '3':
                # code...已退款
       //           $sql="select * from " . $this->pre .'achievement_vip as v left join ' . $this->pre . 'order_info as o on v.order_id=o.order_id' .
       // " where v.user_id='$user_id' and v.status=2  order by created_at desc limit " . $page . ',' . $size;
       $sql = " SELECT * FROM `ecs_achievement_vip` where user_id= '$user_id' and status in('2,9') UNION SELECT * FROM `ecs_achievement_retail` where user_id= '$user_id' and status in(2,9) order by created_at desc limit $page, $size"; 
      
                break;    
            default:
                # code...
                // $sql = "SELECT * FROM " . $this->pre . "achievement_vip WHERE user_id = '$user_id'  order by created_at desc limit " . $page . ',' . $size;
            $sql = " SELECT * FROM `ecs_achievement_vip` where user_id= '$user_id' UNION SELECT * FROM `ecs_achievement_retail` where user_id= '$user_id' order by created_at desc limit $page, $size"; 
    ...
    function selectTotalBonusLog($user_id,$type){
        switch ($type) {
            case '1':
                # code...
                // $sql = "SELECT count(*) as num FROM " . $this->pre . "achievement_vip WHERE user_id = '$user_id' and status=0";
               $sql = " SELECT count(*) as total from (SELECT * FROM `ecs_achievement_vip` where user_id= '$user_id' and status=0 UNION SELECT * FROM `ecs_achievement_retail` where user_id= '$user_id' and status=0 order by created_at desc) as b"; 
                break;
            case '2':
                # code...
                     $sql = " SELECT count(*) as total from (SELECT * FROM `ecs_achievement_vip` where user_id= '$user_id' and status=1 UNION SELECT * FROM `ecs_achievement_retail` where user_id= '$user_id' and status=1 order by created_at desc) as b "; 
                break;
            case '3':
                # code...
                
       //          $sql="select count(v.user_id) as num from " . $this->pre .'achievement_vip as v left join ' . $this->pre . 'order_info as o on v.order_id=o.order_id' .
       // " where v.user_id='$user_id' and v.status=2";
        $sql = " SELECT count(*) as total from (SELECT * FROM `ecs_achievement_vip` where user_id= '$user_id' and  status in('2,9') UNION SELECT * FROM `ecs_achievement_retail` where user_id= '$user_id' and  status in(2,9)  order by created_at desc) as b  "; 
            ...
            default:
                ...
                # code...
                //$sql = "SELECT count(*) as num FROM " . $this->pre . "achievement_vip WHERE user_id = '$user_id'";
                $sql = " SELECT count(*) as total from (SELECT * FROM `ecs_achievement_vip` where user_id= '$user_id' UNION SELECT * FROM `ecs_achievement_retail` where user_id= '$user_id' order by created_at desc) as b "; 
    ...
    function sumTotalBonusLog($user_id,$type){
        switch ($type) {
            case '1':
                # code...
                $sql = "SELECT sum(money) as total FROM " . $this->pre . "achievement_vip WHERE user_id = '$user_id' and status=0";
                $sql1 = "SELECT sum(money) as total FROM " . $this->pre . "achievement_retail WHERE user_id = '$user_id' and status=0";
                break;
            case '2':
                # code...
                $sql = "SELECT sum(money) as totalFROM " . $this->pre . "achievement_vip WHERE user_id = '$user_id' and status=1";
                  $sql1 = "SELECT sum(money) as total FROM " . $this->pre . "achievement_retail WHERE user_id = '$user_id' and status=0";
                break;
            case '3':
                # code...
                
                $sql="select count(v.user_id) from " . $this->pre .'achievement_vip as v left join ' . $this->pre . 'order_info as o on v.order_id=o.order_id' .
       " where v.user_id='$user_id' and o.order_status=2";
       $sql1="select count(v.user_id) from " . $this->pre .'achievement_retail as v left join ' . $this->pre . 'order_info as o on v.order_id=o.order_id' .
       " where v.user_id='$user_id' and o.order_status=2";
        ...
        $res = $this->row($sql);
        $res1 = $this->row($sql1);
        return $res['total']+$res1['total'];
    }
    function getBankCardInfo($bank_id){
        $where['bank_id'] = $bank_id;
        $res = $this->model->table('user_bank_list')->field('user_id,bank_no,bank_realname,bank_name,bank_sub_name')->where($where)->find();
        return $res;
    }
    function  isEnough($amount,$user_id,$type){
        ...
        $res = $this->model->table('account')->field('balance ,bonus_distributor ,bonus_vip,bonus_retail')->where($where)->find();
       
        //1:balance 2:bonus_distributor 3:bonus_vip 4:bonus_retail 5:rank_points 6:pay_points 7:vcoin  8:fish_ticket'
        switch ($type) {
            case '1':
                # code...
            ...
            case '2':
                # code...
                if($amount<=$res['bonus_distributor']){
                    return true;
                }else{
                    return false;
                }
            ...
            case '3':
                ...
                if($amount<=($res['bonus_vip']+$res['bonus_retail'])){
     ...
     * @return  void
     */
    function log_bonus_change($user_id, $bonus_money = 0, $change_desc,$change_type = ACT_OTHER,$account_type = 1) {
        /* 插入帐户变动记录 */
         $sql = "SELECT vip_manage_account FROM " . $this->pre . "users WHERE `user_id` = '$user_id'";
        $info = $this->row($sql);
        $account_log = array(
            'user_id' => $user_id,
            'amount' => $bonus_money,
            'account' => $info['vip_manage_account'],
            'change_time' => time(),
            'change_desc' => $change_desc,
            'change_type' => $change_type,
            'account_type'=> $account_type
...
/*     $where['user_id'] = $user_id;
       ...
       $sql = "SELECT count(*) as num FROM " . $this->pre . "order_info WHERE user_id = '$user_id' and order_status!=6  " . $where ;
       $res = $this->row($sql);
       return $res['num'];
    }
    public function update_allorder($user_id){

        $sql = "select * from " . $this->pre . "order_info where user_id ='$user_id' ";
        $res = $this->query($sql); 
        
        foreach ($res as $key => $value) {
           
           if((time()-$value['add_time'])>3600){
            //超时
            
            $r = model('Users')->cancel_order($value['order_id'], $user_id);
    ...
    public function pay_money($user_id) {
       $where['user_id'] = $user_id;
       $res = $this->model->table('users')->field('vcoin ,fish_ticket')->where($where)->find();
       return $res;
    }
     /**
     * 获取未评价订单的数量
    ...
    public function get_banklist($user_id){
        $this->table = 'user_bank_list';
        $condition['user_id'] = $user_id;
        return $this->select($condition, '', 'add_time desc');
    }
     /**
     * 添加缺货登记记录到数据表
     * @access  public
     ...
     * @return void
     */
    public function add_bank($bank) {
       
        $data['user_id'] = $_SESSION['user_id'];
        $data['bank_no'] = $bank['bank_no'];
        $data['bank_realname'] = $bank['bank_realname'];
        $data['bank_name'] = $bank['bank_name'];
        $data['bank_sub_name'] = $bank['bank_sub_name'];
        $data['add_time'] = time();
        $this->table = 'user_bank_list';
      
        return $this->insert($data);
    }
    public function del_bank($bank_id, $user_id) {
        $this->table = 'user_bank_list';
    ...
    public function user_statistics($user_id){
         $this->table = 'user_statistics';
         $condition['user_id'] = $user_id;
         return $this->find($condition);
    }

    public function user_statistics_month($user_id,$num,$start){

         $sql = 'SELECT * FROM ' . $this->pre . "user_statistics_month WHERE user_id = '$user_id'" .
     ...
     public function total_user_statistics_month($user_id){

         $sql = 'SELECT count(*) as num FROM ' . $this->pre . "user_statistics_month WHERE user_id = '$user_id'";
