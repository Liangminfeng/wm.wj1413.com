...
class GoodsBaseModel extends BaseModel {
            ...
            foreach ($res as $data) {
                switch ($data['act_type']) {
                    ...
                    case GAT_GROUP_BUY: //团购
                        ...
                        $group[$data['act_id']]['start_time'] = $data['start_time'];
                        $group[$data['act_id']]['end_time'] = $data['end_time'];
     ...
     * @param int $goods_id
     */
    function get_groupbuy_info($goods_id){
        $gmtime = time();
        
        $sql = 'SELECT * FROM ' . $this->pre . "goods_activity WHERE is_finished=0 AND start_time <= '$gmtime' AND end_time >= '$gmtime' AND goods_id = '$goods_id' AND act_type = ".GAT_GROUP_BUY;
    ...
    function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array()) {
        ...
        //取得商品促销价格列表
        /* 取得商品信息 */
        $sql = "SELECT g.promote_price, g.shop_price,g.vipintegral,g.integral,g.promote_start_date, g.promote_end_date, " .
     ...
     * @return int
     */
    function get_goods_vip_list($goods_vip,$rtcat_id) {
        if($_SESSION['resource']){
            $sql = "SELECT *  FROM " . $this->pre .  "goods WHERE  is_delete=0 and is_on_sale=1 and  FIND_IN_SET($rtcat_id,rtcat_id) and FIND_IN_SET($_SESSION[resource],areas_id)";
            
            if($goods_vip!=0){
                $sql.= " and goods_vip = '" . $goods_vip . "'";
       
            }
           
        }else{
              $sql = "SELECT *  FROM " . $this->pre .  "goods WHERE  is_delete=0 and is_on_sale=1 and  FIND_IN_SET($rtcat_id,rtcat_id) ";
              if($goods_vip!=0){
                  $sql.= " and goods_vip = '" . $goods_vip . "'";
     ...
     * @return int
     */
    function get_upgrade_goods_vip_list($goods_vip,$rtcat_id) {
        if($_SESSION['resource']){
            $sql = "SELECT *  FROM " . $this->pre .  "goods WHERE  is_delete=0 and is_on_sale=1 and  FIND_IN_SET($rtcat_id,rtcat_id) and FIND_IN_SET($_SESSION[resource],areas_id)";
            
            if($goods_vip!=0){
                $sql.= " and goods_vip = '" . $goods_vip . "'";
                $sql.= " and origin_goods_vip = '" . $_SESSION['user_rank'] . "'";
            }
    ...
    /*根据分类来获取产品列表*/
    function get_goods_list_bycat($rtcat_id){
        $sql = "SELECT *  FROM " . $this->pre .
                "goods WHERE  FIND_IN_SET($rtcat_id,rtcat_id)";
        $result = $this->query($sql);
