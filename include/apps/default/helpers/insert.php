...
function insert_query_info() {
    ...
    $cron_method = empty($cron_method) ? '<img src="api/cron.php?t=' . time() . '" alt="" style="width:0px;height:0px;" />' : '';
...
function insert_cart_info_number() {
    $sql = 'SELECT SUM(goods_number) AS number FROM ' . M()->pre . 'cart ' .
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '" . CART_GENERAL_GOODS . "'"." AND (goods_area = 0 or rtcat_id =2)";
