...
if(($strlen - 2)<0){
    ...
    return $newstrlen == 2 ? $firstStr . str_repeat('***', mb_strlen($user_name, 'utf-8') - 1) : $firstStr . str_repeat("*", $newstrlen) . $lastStr;
...
function get_page_title($cat = 0, $str = '') {
    ...
    if (in_array($controller_name, array('category', 'goods', 'article', 'brand'))) {
        ...
        /* 列车活动 */ 
        elseif ('train' == $controller_name) {
            $page_title = L('train') . '_' . $page_title;
    ...
    if (is_array($links) && is_array($hrefs)) {
        ...
        if (!empty($links) && count($links) == count($hrefs)) {
            ...
            foreach ($links as $key => $val) {
                $msg['url_info'][$key]['name'] = $val;
                $msg['url_info'][$key]['href'] = $hrefs[$key];
            }
       ...
       // $link = empty($links) ? L('back_up_page') : "<input style=margin-top:4rem type=button value=".$links.">";
        ...
        $msg['url_info'][1]['href'] = $href;
        $msg['url_info'][1]['name'] = $link;
...
function set_affiliate($u = '') {
    ...
    if (!empty($_GET['u']) && $config['on'] == 1) {
        if (!empty($config['config']['expire'])) {
            ...
            $_COOKIE['ecshop_affiliate_uid'] = intval($_GET['u']);
...
function return_url($code)
{
       // return __URL__ . '/respond.php?code=' . $code;
        if($code=='alipay'){
            return __URL__.'/respond/alipayreturn?';
        }elseif($code=='wxpay'){
            return __URL__.'/respond/wxpayreturn?';
        }
...
function notify_url($code)
{
        ...
        if($code=='alipay'){
            return __URL__.'/respond/alipaynotify';
        }elseif($code=='wxpay'){

            return __URL__.'/respond/wxpaynotify';
        }
    ...
    if (!empty($info['unionid'])) {
        ...
        if (empty($res['unionid'])) {
            ...
            if (!empty($res['ect_uid'])) {
                // 更新社会化登录用户信息
                $connect_userinfo = model('Users')->get_connect_user($info['openid']);
                if (empty($connect_userinfo)) {
                    M()->table('connect_user')->data(array('open_id' => $info['openid']))->where(array('open_id' => $info['openid']))->update();
                }
...
function progressOrder($order_info,$log_id=false){
    ...
    ->select();
    //團購列車判斷
    if ($order_info['extension_code']=='group_buy') {
        
        $istrainproduct = model('Base')->model->table('goods')
        ->field('train_id')
        ->where('goods_id = ' . $order_goods[0]['goods_id'])
        ->getOne();
        //當是列車產品時購票
        if($istrainproduct>0){
            for ($i = 0; $i < $order_goods[0]['goods_number']; $i ++) {
                $r = model("Train")->buyTicket($order_info['user_id'], $order_goods[0]['goods_id'], '0');
        ...
        model('Base')->model->table('order_info')
        ->data($data)
        ->where($where)
        ->update();
        // code...
        ...
        if ($trainproductstatus) {
            
            $where['order_id'] = $order_info['order_id'];
            $data['order_status'] = OS_HIDE;
            $data['pay_status'] = PS_PAYED;
            ...
            foreach ($order_goods as $key1 => $value1) {
                ...
                if ($istrainproduct > 0) {
                    // 列车产品
                    $trainproduct[] = $order_goods[$key1];
                } else {
                    $commproduct[] = $order_goods[$key1];
                }
                /* 插入订单表前先对原先订单产品进行分组一组是普通产品，一组是列车产品 */
            }
            foreach ($trainproduct as $key2 => $value2) {
                // 列车产品如果是A1个 B11个，那么产生12笔订单
                // 判断产品数量
                // 减团购商品库存
                $goodsinfo = model('Goods')->get_goods_info($value2['goods_id']);
                
                $data3['goods_number'] = $goodsinfo['goods_number'] - $value2['goods_number'];
                model('Base')->model->table('goods')
                ->data($data3)
                ->where('goods_id = ' . $value2['goods_id'])
                ->update();
                
                
                $totalprice = $value2['goods_price'] * $value2['goods_number'];
                $num = $value2['goods_number'];
                for ($i = 1; $i <= $num; $i ++) {
                    $order = $order_info;
                    ...
                    $order['goods_amount'] = $value2['goods_price'];
                    $order['order_amount'] = $value2['goods_price'];
                    do {
                    ...
                    $value2['goods_number'] = 1;
                    // $value3 = model('Common')->filter_field('order_info', $order);
                    model('Base')->model->table('order_goods')
                    ->data($value2)
                    ->insert();
                    // 购买车票
                    $r = model("Train")->buyTicket($order_info['user_id'], $value2['goods_id'], $new_order_id);
                    if ($r['data'] == "TOGIVE") {
                        
                        $where['order_id'] = $new_order_id;
                        // 受赠订单待激活
                        $data['order_status'] = OS_WATTING;
                        model('Base')->model->table('order_info')
                        ->data($data)
                        ->where($where)
                        ->update();
            ...
            if (! empty($commproduct)) {
                ...
                // var_dump($commproduct);exit;
                // 普通产品如果按照原先系统来操作
                // 计算普通产品的货总值
                $totalprice = 0;
                foreach ($commproduct as $key3 => $value3) {
                    
                    // $this->model->table('order_goods')->where(array('order_id'=>$_GET['order_id']))->delete();
                    // foreach commproduct to insert the datatable order_goods
                    $totalprice += $value3['goods_price'] * $value3['goods_number'];
                    // $value3 = model('Common')->filter_field('order_info', $order);
                }
            ...
            // $this->model->table('order_info')->where(array('order_id'=>$_GET['order_id']))->delete();
            // 根据不同订单类型（普通、充值） 跳转
            if (isset($log_id) && ! empty($log_id)) {
                $log_id = intval($log_id);
                $pay_log = model('Base')->model->table('pay_log')
        ...
        //         'orderID' => array('value' => $order_info['order_sn'],'color' => '#FF0000'), //订单号
        //         'orderMoneySum' => array('value' => $order_info['order_amount'],'color' => '#FF0000'), //订单金额
 ...
 * @return mixed
 */
function get_user_kd_canuse($user_id,$user_rank,$order_amount){



    $sql_user_rank = "SELECT kd_ratio,cash_ratio,rank_name FROM " . M()->pre .'user_rank_list' . " WHERE level = '$user_rank'";

    $rs = M()->getRow($sql_user_rank);

    $sql_user = "SELECT pay_points FROM " . M()->pre .'users' . " WHERE user_id = '$user_id'";
    $user = M()->getRow($sql_user);
 

    $kd_canuse = min(($rs['kd_ratio']/100)*$order_amount,$user['pay_points']);
...
function create_invite_code() {
    ...
    $sql_rs1 = "SELECT * FROM " . M()->pre .'users' . " WHERE invite_code = '$d'";
    $rs1 = M()->getRow($sql_rs1);

    $sql_rs2 = "SELECT * FROM " . M()->pre .'users' . " WHERE other_invite_code = '$d'";
    $rs2 = M()->getRow($sql_rs2);
    if($rs1||$rs2){
 ...
 * @param $data
 */
function api_log($data,$return)
{
    $log_info = serialize($data);
    $log_return = print_r($return,true);

    $value['log_time'] = time();
    $value['user_id'] = isset($_SESSION[user_id])?$_SESSION[user_id]:0;
    $value['log_info'] = $log_info;
    $value['ip_address'] = real_ip();
    $value['posturl'] = $data['posturl'];
    $value['log_return'] = $log_return;

    
    model('Base')->model->table('api_log')
        ->data($value)
        ->insert();
}
 ...
 * @param $data
 */
function get_vip_ratio($usersid)
{
...
/*更新用户的入团地址*/
    ...
    function updateUserJoinAddress($user_id,$country_id,$province_id,$city_id,$country,$province,$city){
            ...
            M()->table('users')->data($update_data)->where($where)->update();
    ...
    function updateUserOtherInviteCode($user_id,$other_invite_code){
         ...
         $data3['other_invite_code'] = $other_invite_code;
         model('Base')->model->table('users')
         ->data($data3)
         ->where('user_id = ' . $user_id)
         ->update();
         $sql_user = "SELECT user_id FROM " . M()->pre .'vip_account' . " WHERE vip_manage_account = '$other_invite_code'";
         $user = M()->getRow($sql_user);
         $update_data['parent_id'] = $user['user_id'];
         $where = array('user_id' => $user_id); 
         M()->table('users')->data($update_data)->where($where)->update();

    }
    /*判断商品的地区和来源是否匹配*/
    function judgeResoure($goods_id,$resource = 0){

        if(!$resource){
            /*可以访问*/
            return true;
        }
         $sql_goods = "SELECT areas_id FROM " . M()->pre .'goods' . " WHERE goods_id = '$goods_id'";
         $goods = M()->getRow($sql_goods);
         ...
         if(strpos($goods['areas_id'],$find)===0||strpos($goods['areas_id'],$find)===2){
