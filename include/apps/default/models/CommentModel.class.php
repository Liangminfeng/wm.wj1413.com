...
class CommentModel extends BaseModel {
    ...
    function add_comment($cmt,$recId,$order_id) {
        ...
        /* 保存评论内容 */
        $sql = "INSERT INTO " . $this->pre . "comment(comment_type, id_value, email, user_name, content, comment_rank, add_time, ip_address, status, parent_id, user_id , pictures) VALUES " . "('" . $cmt->type . "', '" . $cmt->id . "', '$email', '$user_name', '" . $cmt->content . "', '" . $cmt->rank . "', " . time() . ", '" . real_ip() . "', '$status', '0', '$user_id','$cmt->pictures')";
        ...
        if($result1){
           ...
           // model('ClipsBase')->log_account_change($user_id, 0, 0, 0, 5, "订单评价送KD豆", ACT_KD);
            model('ClipsBase')->new_log_account_change($user_id,5,"订单评价送鱼宝",ACT_KD,6);
    ...
    function get_comment($goods_id,$type,$pay = 1, $num = 10, $start = 0){
        ...
        $sql = "SELECT c.comment_id,c.email,c.user_name,c.content,c.comment_rank,c.add_time,c.pictures,u.user_avatar,u.nick_name FROM ". $this->pre ."comment c join ".$this->pre."users u on c.user_id=u.user_id WHERE ".
               "c.comment_type = 0 AND c.id_value = '".$goods_id."' AND c.status = 1 $where ORDER BY c.comment_id DESC LIMIT $start , $num";
        $comment_list = $this->query($sql);
        foreach($comment_list as $k => $v){
           
            $comment_list[$k]['nick_name'] =  substr_cut($comment_list[$k]['nick_name']);
			$comment_list[$k]['add_time'] = local_date(C('time_format'), $v['add_time']);
			$comment_list[$k]['reply'] =  $this->reply_content($v['comment_id']);
			if(!empty($v["pictures"])) $comment_list[$k]["imgs"] = explode(";", $v["pictures"]);
        }
        return $comment_list;
    }
