...
class FlowModel extends BaseModel
{
    ...
    function flow_clear_cart_alone()
    {
		...
		FROM " . $this->pre . "cart WHERE session_id = '" . SESS_ID . "'
        ...
        /* 删除 */
        $sql = "DELETE FROM " . $this->pre . "cart WHERE session_id = '" . SESS_ID . "'
    ...
    {
        $sql = "INSERT INTO " . $this->pre . "cart(" . "user_id, session_id, goods_id, goods_sn, goods_name, market_price, goods_price, " . "goods_number, is_real, extension_code, parent_id, is_gift, rec_type ) " . "VALUES('$_SESSION[user_id]', '" . SESS_ID . "', 0, '', '$act_name', 0, " . "'" . (- 1) * $amount . "', 1, 0, '', 0, '$act_id', '" . CART_GENERAL_GOODS . "')";
        $this->query($sql);
    }
    ...
    function flow_available_points()
    {
        ...
        {
            //vip
            $sql = "SELECT SUM(g.vipintegral * c.goods_number) as sum " . "FROM " . $this->pre . "cart AS c, " . $this->pre . "goods AS g " . "WHERE c.session_id = '" . SESS_ID . "' AND c.goods_id = g.goods_id AND c.is_buy=1 AND c.is_gift = 0 AND g.vipintegral > 0 " . "AND c.rec_type = '" . CART_GENERAL_GOODS . "'";
        }else{
            $sql = "SELECT SUM(g.integral * c.goods_number) as sum " . "FROM " . $this->pre . "cart AS c, " . $this->pre . "goods AS g " . "WHERE c.session_id = '" . SESS_ID . "' AND c.goods_id = g.goods_id AND c.is_buy=1 AND c.is_gift = 0 AND g.integral > 0 " . "AND c.rec_type = '" . CART_GENERAL_GOODS . "'";
        }
        
        
        $res = $this->row($sql);
        
        $val = floatval($res['sum']);
    ...
    function add_touch_goods($flow_type, $extension_code)
    {
        ...
        foreach ($arrGoodsid as $goodsid) {
            /* 查询该商品销量 */
            $sql = 'SELECT IFNULL(SUM(g.goods_number), 0) ' . 'as count FROM ' . $this->pre . 'order_info AS o, ' . $this->pre . 'order_goods AS g ' . "WHERE o.order_id = g.order_id " . "AND o.extension_code = '$extension_code'  AND g.goods_id = '" . $goodsid['goods_id'] . "' AND o.pay_status = '2' " . $ext;
            $res = $this->row($sql);
            ...
            if ($flow_type == CART_GROUP_BUY_GOODS) {
                /* 查询该商品销量 */
                $sql = 'SELECT IFNULL(SUM(g.goods_number), 0) ' . 'as count FROM ' . $this->pre . 'order_info AS o LEFT JOIN ' . $this->pre . 'order_goods AS g  ON o.order_id = g.order_id ' . 'LEFT JOIN ' . $this->pre . 'goods_activity as ga ON ga.goods_id = g.goods_id ' . "WHERE o.extension_code = '$extension_code'  AND o.pay_status = 2  AND g.goods_id = '" . $goodsid['goods_id'] . "'" . $ext;
                $res = $this->row($sql);
                $nCount = $this->query('select COUNT(*) from ' . $this->pre . 'touch_goods_activity tga LEFT JOIN ' . $this->pre . 'goods_activity ga ON tga.act_id =ga.act_id  where  ga.goods_id = "' . $goodsid['goods_id'] . '" ');
    ...
    // 余额拆单
    function splitorder($order_id)
    {
    
        $order_info = model('Base')->model->table('order_info')
            ->where('order_id = ' . $order_id)
            ->find();
        progressOrder($order_info);
    }
}
