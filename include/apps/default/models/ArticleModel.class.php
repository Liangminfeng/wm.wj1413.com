...
class ArticleModel extends BaseModel {
     ...
     * @return  array
     */
    function assign_articles($id, $num,$resourcetype) {
        ...
        $articles['cat'] = $cat;
        $articles['arr'] = model('ArticleBase')->get_cat_articles($id, 1, $num,'','',$resourcetype);
    ...
    function get_article_info($article_id) {
        ...
        if ($row !== false) {
            ...
            $row['add_time'] = date("Y-m-d", $row['add_time']); // 修正添加时间显示
    ...
    public function isShowcardCat($cat){
    
        if ($cat == 0) {
            return false;
        }
        
        $sql = 'SELECT cat_id FROM ' . $this->pre . "article_cat where cat_id={$cat} and parent_id = 4";
    ...
    public function sucaiList($talk_id,$sort = 'time'){

      if($sort=='time'){
        if($talk_id){
        $sql = 'SELECT * FROM ' . $this->pre . "talk_sucai where talk_id=".$talk_id." and is_show=1 order by sucai_important DESC,change_time desc";

        $res = $this->query($sql);
        if(empty($res))return false;

        }else{
          $sql = 'SELECT * FROM ' . $this->pre . "talk_sucai where is_show=1 order by sucai_important DESC,change_time desc";
        ...
        if($talk_id){
        $sql = 'SELECT * FROM ' . $this->pre . "talk_sucai where talk_id=".$talk_id." and is_show=1 order by sucai_important DESC,sucai_sort asc";
 
        $res = $this->query($sql);
    
        if(empty($res))return false;
        }else{
          $sql = 'SELECT * FROM ' . $this->pre . "talk_sucai where is_show=1 order by sucai_important DESC,sucai_sort asc";
      ...
      foreach ($res as $key=>$val){
          $res[$key]['sucai_content'] = getEmoji($val['sucai_content']);
      }
      
      return $res;
       
    }
     public function totalsucaiList($talk_id){
      if($talk_id){
        $sql = 'SELECT count(*) as total FROM ' . $this->pre . "talk_sucai where talk_id=".$talk_id." and is_show=1";
        ...
        $sql = 'SELECT * FROM ' . $this->pre . "talk_sucai where is_show=1";
    ...
    public function talkList(){
       ...
       $sql = 'SELECT * FROM ' . $this->pre . "talk where is_show=1 order by sort asc";
    ...
    public function talkinfo($id){
        ...
        $sql = 'SELECT * FROM ' . $this->pre . "talk WHERE talk_id = '" . $id . "'";
    ...
    /*根据传过来的链接，获取信息*/
    public function hrefinfo($href)
    {
        if(strpos($href,"&aid=")){
            $hrefarray = explode("&", $href);
            /**/
            $article_id =  substr($hrefarray[3],4);
            $result = $this->get_article_info($article_id);
       
            $res['pic'] =$result['file_url'];
            $res['file_url'] =$href;
            $res['title'] =$result['title'];
            return $res;
         }elseif(strpos($href,"&id=")){
            $hrefarray = explode("&", $href);
    
            $goods_id =  substr($hrefarray[3],3);

            $result = model('Goods')->get_goods_info($goods_id);
       
            $res['pic'] =$result['goods_thumb'];
            $res['file_url'] =$href;
            $res['title'] =$result['goods_name'];
    
            return $res;
         }elseif(strpos($href,"&topic_id=")){
            $hrefarray = explode("&", $href);
   
            $topic_id =  substr($hrefarray[1],9);

            $result =  $this->model->table('topic')->where('topic_id =' . $topic_id)->find();

            $res['pic'] =$result['topic_img'];
            $res['file_url'] =$href;
            $res['title'] =$result['title'];
            return $res;
