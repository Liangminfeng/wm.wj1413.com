...
class GoodsModel extends BaseModel {
    ...
    function get_goods_info($goods_id) {
        ...
        if ($row !== false) {
            ...
            if($_SESSION['user_vip']){
                ...
                $row['shop_price_formated'] = price_format($row['shop_price']-$row['vipintegral'])."+".$row['vipintegral'].C('integral_name');

            }else{
               

                     $row['shop_price_formated'] = "" ;
            ...
            $row['rank_price_formated'] = price_format($row['rank_price']-$row['integral'])."+".number_format(($row['integral']*100/C(integral_scale)),0).C('integral_name');
           ...
           // $row['integral'] = C('integral_scale') ? round($row['integral'] * 100 / C('integral_scale')) : 0;
            ...
            $row['goods_desc'] = str_replace('src="/images', 'src="'. C('shop_url') .'/images', $row['goods_desc']);
               if($_SESSION['user_vip']){
               $row['integral'] = intval($row['vipintegral'])?intval($row['vipintegral']):0; 
               $row['vip_price'] = $row['shop_price']-$row['vipintegral'];
           }else{
             $row['integral'] = $row['integral'];
           }
            return $row;
        } else {
            return false;
    ...
    function get_goods_fittings($goods_list = array()) {
        ...
        foreach ($res as $key => $value) {
            ...
            $arr[$temp_index]['parent_short_name'] = C('goods_name_length') > 0 ?sub_str($value['parent_name'], C('goods_name_length')) : $value['parent_name']; //配件的基本件显示的名称
            ...
            $arr[$temp_index]['short_name'] = C('goods_name_length') > 0 ?sub_str($value['goods_name'], C('goods_name_length')) : $value['goods_name']; //配件显示的名称
    ...
    /*获取列车各种不同状态下的产品列表*/
    function get_train_goods($status){

        $sql = "SELECT g.goods_name,g.train_id,g.goods_id,g.goods_img,g.shop_price,g.goods_img,t.name,t.start_time,t.end_time,t.create_time,t.num,t.total,(t.num/t.total)*100 as percent,t.total-t.num as leftnum,t.description " . "FROM " . $this->model->pre . "goods AS g, " . $this->model->pre . "train AS t " . "WHERE g.train_id =t.train_id AND t.status = '".$status."'";
      
        $res = $this->query($sql);
        return $res;
    }
    function get_best_goods(){
        $sql = "SELECT * FROM " . $this->model->pre . "goods " .  " WHERE is_best =1 and is_on_sale=1  ORDER BY RAND() limit 0,4";
        //SELECT * FROM ecs_goods WHERE is_best =1 and is_on_sale=1 and goods_area = 2 ORDER BY RAND() limit 0,4
        $res = $this->query($sql);
        ...
        foreach ($res as $key => $value) {
          ...
          if($_SESSION['user_vip']){
                if($value['vipintegral'] ==0.00){
                    $value['vipintegral'] = 0;
                }
               $res[$key]['integral'] = $value['vipintegral']; 
               $res[$key]['vip_price'] = $value['shop_price']-$value['vipintegral'];
           }else{
            if($value['vipintegral'] ==0.00){
                    $value['vipintegral'] = 0;
                }
            $res[$key]['integral'] = $value['integral'];
           }
            
            

            $res[$key]['shop_price'] = $value['shop_price'];
            $res[$key]['sales_count'] =  model('GoodsBase')->get_sales_count($value['goods_id']);
            //$res[$key]['new_shoprice'] = price_format($value['shop_price']-$value['integral'])."+".number_format(($value['integral']*100/C(integral_scale)),0)."积分";
            $res[$key]['new_shoprice'] = $value['shop_price'];
     ...
     function apigetgoods($where){
        $sql = "SELECT * FROM " . $this->model->pre . "goods " .  $where;
    ...
    function get_yongjin($goods_id){
        $sql = "SELECT * FROM " . $this->model->pre . "goods " .  " WHERE goods_id =".$goods_id;
        $res = $this->query($sql);
        
        $affiliate = unserialize(C('affiliate'));
        if( $affiliate['config']['is_dependent']){
                $yongjin = ($res['0']['indepent_bonus'] * $affiliate['item'][0]['level_money'])/100;
               // $res[$key]['afflicate'] = $value['indepent_bonus'];
            }else{
                $yongjin = ($res['0']['indepent_bonus'] * $affiliate['item'][0]['level_money'])/100;
            }
            return $yongjin;
    ...
    function get_hot_goods(){
      ...
      //round($money * $affiliate['item'][0]['level_money'], 2);
        $sql = "SELECT * FROM " . $this->model->pre . "goods " .  " WHERE is_hot =1 AND is_delete=0 ORDER BY RAND() limit 0,10";
        ...
        foreach ($res as $key => $value) {
            ...
            //$res[$key]['new_shoprice'] = price_format($value['shop_price']-$value['integral'])."+".number_format(($value['integral']*100/C(integral_scale)),0)."积分";
            $res[$key]['new_shoprice'] = $value['shop_price'];
            $res[$key]['afflicate'] = ($value['shop_price'])*$affiliate['config']['level_money_all']*$affiliate['item'][0]['level_money']/100;
          
            if( $affiliate['config']['is_dependent']){
                $res[$key]['afflicate'] = ($value['indepent_bonus'] * $affiliate['item'][0]['level_money'])/100;
               // $res[$key]['afflicate'] = $value['indepent_bonus'];
            }else{
                $res[$key]['afflicate'] = ($res[$key]['afflicate'] * $affiliate['item'][0]['level_money'])/100;
            }
