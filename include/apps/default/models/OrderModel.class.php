...
class OrderModel extends BaseModel {
     ...
     * @return  array   配送方式数组
     */
    function available_payment_list_new($support_cod, $cod_fee = 0, $is_online = false,$user_id) {
        $sql = 'SELECT pay_id, pay_code, pay_name, pay_fee, pay_desc, pay_config, is_cod' .
                ' FROM ' . $this->pre .'payment WHERE enabled = 1 ';
        if (!$support_cod) {
            $sql .= 'AND is_cod = 0 '; // 如果不支持货到付款
        ...
        $sql .= 'ORDER BY pay_order'; // 排序
        $res = $this->query($sql);
        //$userinfo = $this->model->table('users')->field('user_money')->where("user_id = '" . $user_id . "' ")->getOne();
        ...
        foreach ($res as $key => $row) {
            ...
            if ($row['pay_id'] == '2') {
                ...
                $row['user_money'] = price_format($balancemoney, false);
            }
            $modules[] = $row;
        }
        if (isset($modules)) {
    ...
    function order_info($order_id, $order_sn = '') {
        ...
        //     $order['formated_pay_time'] = date("Y-m-d H:i:s", $order['pay_time']);
        //     $order['formated_shipping_time'] = date("Y-m-d H:i:s", $order['shipping_time']);
        //     $order['formated_confirm_receive_time'] = date("Y-m-d H:i:s", $order['confirm_receive_time']);
        // }
    ...
    function order_goods($order_id) {
        ...
        $sql = "SELECT og.rec_id, og.goods_id, og.goods_name, og.goods_sn, og.market_price, og.goods_number, " .
                ...
                "og.goods_price * og.goods_number AS subtotal, og.extension_code, g.goods_thumb,g.goods_desc,g.goods_brief,g.goods_pv,g.goods_vip " .
     ...
     */
    function cart_goods($type = CART_GENERAL_GOODS,$cart_value,$goods_area = 0) {
      
        if(empty($cart_value)){
            if(strpos($_REQUEST['goods_area'],",")){
            
                $sql = "SELECT ca.rec_id, ca.user_id, ca.goods_id, ca.goods_name, ca.goods_sn, ca.goods_number, " .
                "ca.market_price, ca.goods_price, ca.goods_attr, ca.is_real, ca.extension_code, ca.parent_id, ca.is_gift, ca.is_shipping, g.goods_thumb, g.integral,g.goods_vip,g.goods_name,g.vipintegral,g.goods_brief,g.goods_area,g.goods_pv,g.rtgoods_pv,g.vipgoods_pv,g.rtcat_id,g.lscat_id,g.train_id," .
                "ca.goods_price * ca.goods_number AS subtotal " .
                "FROM " . $this->pre .
                "cart AS ca, " .$this->pre ."goods AS g " ."WHERE session_id = '" . SESS_ID . "' " .
                "AND rec_type = '$type'" . 
                " AND g.goods_area = '$goods_area' " .
                "AND ca.goods_id = g.goods_id and ca.is_buy=1 order by ca.addcarttime desc";
            }else{
                ...
                "cart AS ca, " .$this->pre ."goods AS g " ."WHERE session_id = '" . SESS_ID . "' " .
                "AND rec_type = '$type'" . 
                " AND find_in_set('$goods_area',g.goods_area) " . 
                "AND ca.rec_id in ($cart_value)" . 
                "AND ca.goods_id = g.goods_id and ca.is_buy=1 order by ca.addcarttime desc";
        ...
        foreach ($arr as $key => $value) {
            $arr[$key]['train_id'] = $value['train_id'];
     ...
     * @return  array   购物车商品数组
     */
    function new_cart_goods($type = CART_GENERAL_GOODS,$cart_value,$goods_area = 0) {
      
        if(empty($cart_value)){
      
            $sql = "SELECT ca.rec_id, ca.user_id, ca.goods_id, ca.goods_name, ca.goods_sn, ca.goods_number, " .
                "ca.market_price, ca.goods_price, ca.goods_attr, ca.is_real, ca.extension_code, ca.parent_id, ca.is_gift, ca.is_shipping, g.goods_thumb, g.integral,g.goods_vip,g.goods_name,g.goods_brief,g.goods_pv,g.rtcat_id,g.lscat_id,g.goods_area,g.train_id," .
                "ca.goods_price * ca.goods_number AS subtotal " .
                "FROM " . $this->pre .
                "cart AS ca, " .$this->pre ."goods AS g " ."WHERE session_id = '" . SESS_ID . "' " .
                "AND rec_type = '$type'" . 
               
                "AND ca.goods_id = g.goods_id and ca.is_buy=1 and ca.goods_area = '".$goods_area."' order by ca.addcarttime desc";
            }else{
     ...
     * @return  array   购物车商品数组
     */
    function newnew_cart_goods($type = CART_GENERAL_GOODS,$cart_value,$goods_area = 0) {
      
        if(empty($cart_value)){
      
            $sql = "SELECT ca.rec_id, ca.user_id, ca.goods_id, ca.goods_name, ca.goods_sn, ca.goods_number, " .
                "ca.market_price, ca.goods_price, ca.goods_attr, ca.is_real, ca.extension_code, ca.parent_id, ca.is_gift, ca.is_shipping, g.goods_thumb, g.integral,g.goods_vip,g.goods_name,g.goods_brief,g.goods_pv,g.rtgoods_pv,g.vipgoods_pv,g.rtcat_id,g.lscat_id,g.goods_area,g.train_id," .
                "ca.goods_price * ca.goods_number AS subtotal " .
                "FROM " . $this->pre .
                "cart AS ca, " .$this->pre ."goods AS g " ."WHERE session_id = '" . SESS_ID . "' " .
                "AND rec_type = '$type'" . 
               
                "AND ca.goods_id = g.goods_id and ca.is_buy=1 and (ca.goods_area=0 or ca.rtcat_id=2) order by ca.addcarttime desc";
            }else{
     ...
     * @return  array   购物车商品数组
     */
    function cart_goods_group($type = CART_GENERAL_GOODS,$cart_value) {
     
        if(empty($cart_value)){
            $sql = "SELECT ca.rec_id, ca.user_id, ca.goods_id, ca.goods_name, ca.goods_sn, ca.goods_number, " .
                "ca.market_price, ca.goods_price, ca.goods_attr, ca.is_real, ca.extension_code, ca.parent_id, ca.is_gift, ca.is_shipping, g.goods_thumb, g.train_id," .
                "ca.goods_price * ca.goods_number AS subtotal " .
                "FROM " . $this->pre .
                "cart AS ca, " .$this->pre ."goods AS g " ."WHERE session_id = '" . SESS_ID . "' " .
                "AND rec_type = '$type'" . 
 ...
 /*清空购物车购买商品*/
    function clear_cart_buy($goods_id) {
        $sql = "DELETE FROM " . $this->pre .
            "cart WHERE goods_id IN ($goods_id) AND session_id = '" . SESS_ID ."' AND is_buy = 1";
        $this->query($sql);
    }
    /**
     * 取得购物车总金额
     ...
     * @return  boolean
     */
    function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0,$goods_area = 0,$order_type = 0) {
        ECTouch::err()->clean();
        $_parent_id = $parent;

        /* 取得商品信息 */
        $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.rtcat_id,g.goods_pv,g.rtgoods_pv,g.vipgoods_pv,g.is_real, " .
        ...
        if($goods['goods_pv'] =='-1'){
                ...
                /*-1并且又是经销商商品*/
                if($goods_area==1){
                     $goods['goods_pv']  =  ($goods['shop_price']/10)*0.7;
                 }else{
                     $goods['goods_pv']  =  $goods['shop_price'];
                 }
            ...
            if ($row) { //如果购物车已经有此物品，则更新
                ...
                if (C('use_storage') == 0 || $num <= $goods_storage) {
                    ...
                    $sql = "UPDATE " . $this->pre . "cart SET goods_number = '$num'" .
                            " , goods_price = '$goods_price'" . ",addcarttime = '$addcarttime'".
    ...
    function clear_cart($type = CART_GENERAL_GOODS,$cart_value) {
        ...
        if(empty($cart_value)){
            $sql = "DELETE FROM " . $this->pre .
                "cart WHERE user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '$type' and is_buy=1 ";
           
            }else{
                $sql = "DELETE FROM " . $this->pre .
                "cart WHERE user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '$type' AND rec_id in ($cart_value) and is_buy=1 ";
            }
    ...
    function getuserordernum($user_id) {
        $sql = "SELECT count(*) as num FROM " . $this->pre .
                "order_info WHERE user_id = '$user_id' and pay_status=2";
        $res = $this->row($sql);
        return $res['num'] ;
    }
    ...
    function use_bonus($bonus_id, $order_id) {
        $sql = "UPDATE " . $this->pre .
                "user_bonus SET order_id = '$order_id', used_time = '" . time() . "' " .
    ...
    function order_refund($order, $refund_type, $refund_note, $refund_amount = 0) {
        ...
        if (1 == $refund_type) {
           // model('ClipsBase')->log_account_change($user_id, $amount, 0, 0, 0, $change_desc);
            model('ClipsBase')->new_log_account_change($user_id,$amount,$change_desc,ACT_OTHER,1);
            return true;
     ...
     * @return  array
     */
    function get_cart_goods($is_buy = '',$goods_area = 0) {
        /* 初始化 */
        $goods_list = array();
        $total = array(
        ...
        $where = " AND (goods_area = 0 or rtcat_id = 2)" ;
        if($is_buy){
         $where .= " AND is_buy = 1" ;
        }
        /* 循环、统计 */
        $sql = "SELECT *, IF(parent_id, parent_id, goods_id) AS pid " .
            " FROM " . $this->pre . "cart " .
            " WHERE session_id = '" . SESS_ID . "'   AND rec_type = '" . CART_GENERAL_GOODS . "'". $where.
            " ORDER BY addcarttime desc";
...
//    }
    ...
    function cancelOrder($user_id,$order_type,$order_id){

        $sql = "UPDATE `ecs_order_info` SET `order_status` = 2  WHERE user_id = '$user_id' and order_type = '$order_type' and order_id<>'$order_id'";
