...
//        $goodslist = $this->category_get_goods();
    ...
    public function async_list()
    {
        ...
        die(json_encode(array('list' => $goodslist),JSON_UNESCAPED_UNICODE));
        exit();
    }
    ...
    public function keywords()
    {
        ...
        if ($keyword != '') {
            ...
            if (!empty($_COOKIE['ECS']['keywords'])) {
                ...
                setcookie('ECS[keywords]', implode(',', $history), time() + 3600 * 24 * 30);
            } else {
                setcookie('ECS[keywords]', $keyword, time() + 3600 * 24 * 30);
            }
    ...
    private function parameter()
    {
        ...
        setcookie('ECS[display]', $display, time() + 86400 * 7);
    ...
    private function category_get_goods()
    {
       ...
       if($sort == 'goods_id'){
        ...
        $sql = 'SELECT g.goods_id,g.integral,g.vipintegral,g.areas_id,g.specialicon, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, g.last_update,' . "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, g.goods_number, " .
            'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' . 'FROM ' . $this->model->pre . 'goods AS g '  . ' LEFT JOIN ' . $this->model->pre . 'member_price AS mp ' . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " . "WHERE $where $this->ext ORDER BY $sort $this->order  LIMIT $start , $this->size";
