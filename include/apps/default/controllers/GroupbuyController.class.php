...
class GroupbuyController extends CommonController {
    ...
    public function info() {
       ...
       // $this->assign('sales_count', model('GoodsBase')->get_sales_count(1));
        ...
        $this->assign('sales_count', model('GoodsBase')->get_sales_count($goods_id));
        ...
        $this->assign('now_time', time());           // 当前系统时间
        ...
        $this->assign('touch_img',$touch_img);
        $this->assign('act_desc',$act_desc);
    ...
    public function buy() {
        ...
        $goods_info = model('Goods')->get_goods_info($group_buy['goods_id']);
            //增加是否可以购买列车产品判断
            
            if($_SESSION ['user_id']>0&&$goods_info['train_id']>0){

                $allowbuy = model("Train")->allowBuy($_SESSION ['user_id'],$group_buy['goods_id'],$number);
         
                if($allowbuy!="ok"){
                    show_message($allowbuy, '', '', 'error');
        ...
        /* 更新：清空购物车中所有团购商品 */
        model('Order')->clear_cart(CART_GROUP_BUY_GOODS,false);
