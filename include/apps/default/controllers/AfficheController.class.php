...
class AfficheController extends CommonController
{
    ...
    public function index()
    {
        ...
        if ($act == 'js') {
            ...
            /* 取得广告的信息 */
            $sql = 'SELECT ad.ad_id, ad.ad_name, ad.ad_link, ad.ad_code ' . 'FROM ' . $this->model->pre . 'ad AS ad ' . 'LEFT JOIN ' . $this->model->pre . 'ad_position AS p ON ad.position_id = p.position_id ' . "WHERE ad.ad_id = '$ad_id' and " . time() . " >= ad.start_time and " . time() . "<= ad.end_time";
            ...
            if ($ad_id == '-1') {
                ...
                if(!empty($_SESSION["user_id"])&&!strpos($uri, "&u=")){
                    $uri.="&u=".$_SESSION["user_id"];
                }
                $uri = str_replace('&amp;', '&', $uri);
                $this->redirect($uri);
                exit();
