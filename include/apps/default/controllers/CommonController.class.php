...
class CommonController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        if(!is_wechat_browser()&&!in_array($route, ['index/towechat','train/adlogin',"qiniu/token"])&&empty($_SESSION['superadminlogin'])){
            $logouturl = url('index/towechat');
        ...
        if($_SESSION['user_id']){
            ...
            $where1['user_id'] = $_SESSION['user_id'];
             ...
             $r = $this->model->table('users')
                    ->field('user_id,user_rank,lang,user_vip,resource,status')
                    ->where($where1)
                    ->find();
             if($r['status']==9){
        ...
        if($_SESSION['openid']&&!$_SESSION['user_id']){
            ...
            //强制跳转
            if(ENV!=="DEBUG"&&empty($_SESSION["superadminlogin"])&&!in_array(strtolower(CONTROLLER_NAME), ['oauth','respond','public','api'])&&!in_array($route, ['train/adlogin','affiliate/getajaxgoodslist']) ){

             //  $this->pc_jump_code();
            }
            //微信登录
            $allow = ["user/createuseraccount","goods/index","user/linkuseraccount","api/index","api/jssdk","api/qrcode","api/getconfig","api/register","api/setsession","api/login","api/dologin","api/checklogin","api/bind","api/getsalts","api/getsalts","api/getresource","api/getkey"];
            if(WECHAT_MODE&&!in_array($route, $allow)){
                $this->wechat_init();
            }
        ...
        self::$view->assign('meta_description', $this->formatDescription(C('shop_desc')));
        ...
        $this->assign("footer_index",strtolower(CONTROLLER_NAME));
    ...
    protected function ecshop_init()
    {
        ...
        /* if(!empty($_SESSION["user_id"])){
            ...
            if(!isset($shouzengs))  {
                $shouzengs =   count(model("Train")->getUserUnactiveTicket($_SESSION['user_id']));
                self::$cache->setValue("shou_zeng_order_{$_SESSION["user_id"]}",$shouzengs,1800);
            }
            
            $shouzengts = self::$cache->getValue("shou_zeng_order_t_{$_SESSION["user_id"]}");

            if(!isset($shouzengts))  {
                $shouzengts =   count(model("Train")->getUserUncheckTicket($_SESSION['user_id']));
                self::$cache->setValue("shou_zeng_order_t_{$_SESSION["user_id"]}",$shouzengts,1800);
            }
            
            if($shouzengs>0||$shouzengts>0){
             
                $route = strtolower(CONTROLLER_NAME."/".ACTION_NAME);
                $allow = ["index/index","user/modifygivenorderaddress","train/adlogin","public/region","index/ticketconfirm"];
        ...
        if (defined("DEBUG_MODE")&&(DEBUG_MODE & 2) == 2) {
        ...
        if (empty($_SESSION["user_id"])&&! empty($_COOKIE['ECS']['user_id']) && ! empty($_COOKIE['ECS']['password'])) {
            ...
            $row = $this->model->table('users')
                ->field('user_id, user_name, password,email,nick_name,rank_points,lang')
                ->where($where)
                ->find();
            if ($row) {
                $_SESSION['user_id'] = $row['user_id'];
                ...
                $rank = $this->model->table('user_rank')
                ->where(["rank_id"=>2])
                ->find();
                $_SESSION['chuangke'] = $row['rank_points']>$rank["min_points"]?true:false;
     ...
     * @return
     */
    protected function line_init()
    {
        // 微信oauth处理
        
        $this->init_oauth();
        
        if(class_exists('LineController') && is_line_browser()){
            //是否显示关注按钮
            // $condition['openid'] = !empty($_SESSION['openid']) ? $_SESSION['openid'] : 0;
            $condition['ect_uid'] = !empty($_SESSION['user_id']) ? $_SESSION['user_id'] : 0;
            $userinfo = $this->model->table('line_user')->field('subscribe')->where($condition)->find();
            $_SESSION['subscribe'] = $userinfo['subscribe'];
        }
        // 关注按钮 是否显示
        $this->assign('subscribe', $_SESSION['subscribe']);
        ...
        // 判断微信通 用于JSSDK
        $is_line = class_exists('LineController') && is_line_browser() ? 1 : 0;
    ...
    public function pc_jump_code()
    {
        ...
        {
             $route = strtolower(CONTROLLER_NAME."/".ACTION_NAME);
             $allow = ["user/createuseraccount","user/linkuseraccount","api/index","api/jssdk","api/qrcode","api/getconfig","api/register","api/setsession","api/login","api/dologin","api/checklogin","api/bind","api/getsalts","api/getsalts","api/getresource","api/getkey"];
    ...
    public function formatShareImage($url){
        if(strpos($url, "imageView2")===false)$url.="?imageView2/1/w/60/h/60/q/100|imageslim/";
        return $url;
    }
    
    public function getShortUrl($url){
        return $url;
       /* $postUrl = "https://firebasedynamiclinks.googleapis.com/v1/shortLinks?key=AIzaSyCyP5D6Ry-jEoZ27Ony80Z5co-vrI46sUw";
       $data = array(
           "dynamicLinkInfo" => array(
               "link" => $url,
               "dynamicLinkDomain"  => "vp9t3.app.goo.gl"
           ),
           "suffix"=>["option"=>"SHORT"]
       ); 
       $ret = json_decode(Http::doPost($postUrl,$data,5,"Content-Type: application/json",'json'),true);
       return  $ret["shortLink"]; */
        $longurl = "http://api.u6.gg/api.php?url=".urlencode($url);
        $r=Http::doGet($longurl);
        $r= substr($r, 9);
    ...
    private function init_oauth()
    {
        ...
        if ($browers && (empty($_SESSION['openid']) ) && strtolower(CONTROLLER_NAME) != 'oauth') {
            $sql = "SELECT `auth_config` FROM " . $this->model->pre . "touch_auth WHERE `from` = 'wechat' ";
            ...
            if ($auth_config) {
                ...
                if(strpos($back_url, "logout")){
                    $back_url =  url('index/index');
                }
                
                setcookie('ecshop_affiliate_uid', $_GET['u'], time()+1800);
                $url = url('oauth/index', array('type' => $browers, 'back_url' => $back_url ));
     ...
     * @param number $code
     */
    protected function jsonResult($data,$message='',$code=200){
     ...
     * @param number $code
     */
    protected function api_jsonp_encode($data,$message='',$code=200){
    ...
    function jsonPError($message,$code=400){
        $result = array(
            "msg"   => $message,
            "code"  => $code
        );
        $callback = $_GET['callback'];
         $tmp= json_encode($result,JSON_UNESCAPED_SLASHES);
         echo $callback . '(' . $tmp .')';  //返回格式，必需
