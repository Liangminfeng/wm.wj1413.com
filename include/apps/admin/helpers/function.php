...
function page_and_size($filter)
{
    ...
    /* 每页显示 */
    $filter['page'] = (empty($_REQUEST['page']) || intval($_REQUEST['page']) <= 0) ? 25 : intval($_REQUEST['page']);
...
function update_goods($goods_id, $field, $value)
{
    if ($goods_id) {
        ...
        $sql = "UPDATE " . M()->pre . 'goods ' . " SET $field = '$value' , last_update = '" . time() . "' " . "WHERE goods_id " . db_create_in($goods_id);
        return M()->query($sql);
    } else {
        return false;
    }
