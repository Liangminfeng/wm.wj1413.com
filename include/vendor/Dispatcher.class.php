...
class Dispatcher {
    ...
    static public function dispatch() {
        ...
        if(strpos(__SELF__, "index.php")>0||strlen(__SELF__)<=1){
            // 获取模块名称
            ...
            $uri = __SELF__;
            if(strpos(__SELF__, "?")>0) $uri = substr($uri, 0,strpos($uri, "?"));
            $route = explode("/", $uri);
            $r = count($route);
            if($r==1){
                ...
                define('APP_NAME', DEFAULT_APP);
                // 获取控制器和操作名
                define('CONTROLLER_NAME',   parse_name($route[1],1));
                define('ACTION_NAME',  DEFAULT_ACTION);
            }elseif ($r==3){
                define('APP_NAME', DEFAULT_APP);
                // 获取控制器和操作名
                ...
                // 获取控制器和操作名
                define('CONTROLLER_NAME',   parse_name($route[2],1));
                define('ACTION_NAME',  $route[3]);
            }
