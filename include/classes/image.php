...
require_once ROOT_PATH.'include/libraries/Myqiniu.class.php';
...
class image
{
    ...
    private $appkey = "bZks39r-MN2-KlD32CZgmRcoceWz5Imb4273jcPB";
    private $appsecret = "5TNupN2zPqczX8C0mu14dP3fugJVw0pU6SvoFhxF";
    
    private $domain = "";
    private $region = "";
    private $defaultBucet = "";
...
/*         "hn"  =>  array(
    ...
    {
        $config = null;
        $config = $this->config[$this->kongjian];
        
        $this->domain = $config["domain"];
        $this->region = $config["region"];
        $this->defaultBucet = $config["bucket"];
    ...
    {   
        ...
        if(is_array($upload["name"])){
            ...
            $files = array();
            foreach ($upload["name"] as $key=>$val){
                $files[$key]["name"] =  $val;
                $files[$key]["type"] =  $upload["type"][$key];
                $files[$key]["tmp_name"] =  $upload["tmp_name"][$key];
                $files[$key]["size"] =  $upload["size"][$key];
        ...
        foreach ($files as $key=>$file){
            ...
            if (empty($img_name))
            {
                $img_name = date("Ymd")."/".$key.time().rand(1,100). $this->get_filetype($file['name']);
            }
            if (!$this->check_img_type($file['type']))
            {
            ...
            if (!check_file_type($file['tmp_name'], $img_name, $allow_file_types))
            {
                $this->error_msg = L('invalid_upload_image_type');
                $this->error_no  =  ERR_INVALID_IMAGE_TYPE;
                return false;
            ...
            $ret = Myqiniu::uploadFile($file,$img_name);
            if(!$ret["state"]){
                $this->error_msg = $ret["msg"];
     ...
     * @return      mix         如果成功则返回文件名，否则返回false
     */
    function upload_image_old($upload, $dir = '', $img_name = '')
    {
        /* 没有指定目录默认为根目录images */
        if (empty($dir))
        {
    ...
    function qiniu_copy($from_key,$to_key){
        $from_key = parse_url($from_key)["path"];
        if(strpos($from_key, "/")===0)$from_key = substr($from_key, 1);
        if(strpos($from_key, "?")!=0)$from_key = substr($from_key, 0,stripos($from_key , "?"));
  
        $auth = new Auth($this->appkey, $this->appsecret);
        $manager = new BucketManager($auth);
       
        $ret = $manager->copy($this->defaultBucet, $from_key, $this->defaultBucet, $to_key);


        if($ret!=null){
            $this->error_msg = $ret->message();
            $this->error_no  = ERR_UPLOAD_FAILURE;
            return false;
        }

        return $this->domain."/".$to_key;
    }
}
