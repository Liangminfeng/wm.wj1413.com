...
class passport
{
    ...
    public function __construct($cfg)
    {
        ...
        $this->field_vip_manage_account = 'vip_manage_account';
        $this->field_user_vip = 'user_vip';
        $this->field_reg_date = 'reg_time';
        $this->field_passwd_question = 'passwd_question';
        $this->need_sync = false;
        $this->field_nick_name = 'nick_name';
        $this->is_ecshop = 1;
...
//             }
    ...
    public function edit_user($cfg)
    {
        ...
        // }
          ...
          $new_compile_password = $this->new_compile_password(array(
                'password' => $cfg['password']
            ));
          if ((! empty($cfg['md5password'])) && $this->field_pass != 'NULL') {
            $values[] = $this->field_pass . "='" . $new_compile_password . "'";
        }
...
".$this->field_name." <> '".$cfg['post_username']."'";
        ...
        if ((! empty($cfg['nick_name'])) && $this->field_nick_name != 'NULL') {
               $values[] = $this->field_nick_name . "='" . $cfg['nick_name'] . "'";
        }
      
    
        if ($values) {
            $sql = "UPDATE ecs_" . $this->user_table . " SET " . implode(', ', $values) . " WHERE " . $this->field_name . "='" . $cfg['post_username'] . "' LIMIT 1";
         
            /*把更改的信息发聩到VIP中心api/user/update*/
        
            $where['user_name'] = $cfg['post_username'];

            $res = $this->db->table('users')->field('user_id,user_vip,resource')->where($where)->find();         
           
            $where1['user_id'] = $res['user_id'];
         
            //$result = $this->db->table('vip_account')->field('vip_manage_account')->where($where1)->find();    
            ...
            if($res['resource']){
               ...
               /*  if($result){
                            $account =   $result['vip_manage_account'];
                     }else{
          ...
          /*     if($res['resource']==1&&!empty($anewOrderdata)){

                
                $ret1 = post_log($anewOrderdata,API_URL."/api/user/update",5,$res['user_id']);
    ...
    public function remove_user($id)
    {
        ...
        if ($this->need_sync || (isset($this->is_ecshop) && $this->is_ecshop)) {
            ...
            if ($col) {
                ...
                $this->db->query($sql);
                //新==========
                $sql = "DELETE FROM {pre}account  WHERE " . db_create_in($col, 'user_id'); // 删除微信用户
                $this->db->query($sql);
                $sql = "DELETE FROM {pre}achievement_retail  WHERE " . db_create_in($col, 'user_id'); // 删除微信用户
                $this->db->query($sql);
                $sql = "DELETE FROM {pre}achievement_vip  WHERE " . db_create_in($col, 'user_id'); // 删除微信用户
                $this->db->query($sql);
                $sql = "DELETE FROM {pre}vip_account  WHERE " . db_create_in($col, 'user_id'); // 删除微信用户
                $this->db->query($sql);
    ...
    public function get_profile_by_name($username)
    {
        ...
        $sql = "SELECT " . $this->field_id . " AS user_id," . $this->field_name . " AS user_name," . $this->field_user_vip . " AS user_vip," . $this->field_vip_manage_account . " AS vip_manage_account," . $this->field_email . " AS email," . $this->field_mobile . " AS mobile," . $this->field_gender . " AS sex," . $this->field_bday . " AS birthday," . $this->field_reg_date . " AS reg_time, " . $this->field_passwd_question . " AS passwd_question," . $this->field_pass . " AS password " . " ,resource AS resource FROM {pre}" . $this->user_table . " WHERE " . $this->field_name . "='$post_username'";
