...
function load_config()
{
    ...
    if ($data === false)
    {
        ...
        //限定语言项 
        $lang_array = array('zh_cn', 'zh_tw', 'en_us', 'tw_cn');
 ...
 * @return  void
 */
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER , $vcoin = 0 , $flish_ticket = 0)
{
    ...
    $sql = "UPDATE " . $global->ecs->table('users') .
            ...
            " vcoin = vcoin + ('$vcoin')," .
            " fish_ticket = fish_ticket + ('$flish_ticket')" .
            " WHERE user_id = '$user_id' LIMIT 1";
    $global->db->query($sql);
}
    /*20190515新的记录函数*/
    function new_log_account_change($user_id, $amount = 0,$change_desc = '', $change_type = ACT_OTHER, $account_type = 1) {

         $global = getInstance();
    $sql = 'SELECT vip_manage_account FROM ' . $global->ecs->table('users') . " WHERE user_id = '$user_id'";
    $account = $global->db->GetOne($sql);
        $accountisexist =  $global->db->GetOne('SELECT user_id FROM ' . $global->ecs->table('account') . " WHERE user_id = '$user_id'");
        ...
        if(!$accountisexist){
            ...
            'user_id' => $user_id
             ...
             $global->db->autoExecute($global->ecs->table('account'), $accountdata, 'INSERT');
        }
        /* 插入帐户变动记录 */
        $account_log = array(
            'user_id' => $user_id,
        ...
        */
        ...
        switch ($account_type) {
            case '1':
                # balance...
                $sql = "UPDATE " . $global->ecs->table('account')." SET balance = balance + ('$amount')" .
             ...
             case '2':
                # bonus_distributor...
                $sql = "UPDATE " . $global->ecs->table('account')." SET bonus_distributor = bonus_distributor + ('$amount')" .
             ...
             case '3':
                # bonus_vip...
                 $sql = "UPDATE " . $global->ecs->table('account')." SET bonus_vip = bonus_vip + ('$amount')" .
             ...
             case '4':
                # bonus_retail...
                $sql = "UPDATE " . $global->ecs->table('account')." SET bonus_retail = bonus_retail + ('$amount')" .
             ...
             case '5':
                # rank_points...
                $sql = "UPDATE " . $global->ecs->table('account')." SET rank_points = rank_points + ('$amount')" .
             ...
             case '6':
                # pay_points...
                $sql = "UPDATE " . $global->ecs->table('account')." SET pay_points = pay_points + ('$amount')" .
                ...
                break;
            case '7':
                $sql = "UPDATE " . $global->ecs->table('account')." SET vcoin = vcoin + ('$amount')" .
            ...
            case '8':
                # fish_ticket...
            $sql = "UPDATE " . $global->ecs->table('account')." SET fish_ticket = fish_ticket + ('$amount')" .
            ...
            case '9':
                # frozen_money“需要审核”..
                $sql = "UPDATE " . $global->ecs->table('account')." SET frozen_money = frozen_money + ('$amount')" .
            ...
            case '10':
                # 需要审核的“已提现”...
                 $sql = "UPDATE " . $global->ecs->table('account')." SET withdraw = withdraw + ('$amount')" .
            ...
            case '11':
                # 零钱账户..
                   ...
                   $sql = "UPDATE " . $global->ecs->table('account')." SET small_change = small_change + ('$amount')" .
            ...
            case '12':
                ...
                $sql = "UPDATE " . $global->ecs->table('account')." SET lxpoint = lxpoint + ('$amount')" .
            ...
            case '14':
                ...
                $sql = "UPDATE " . $global->ecs->table('account')." SET train_money = train_money + ('$amount')" .
    ...
    function finduseraccountinfo($user_id,$account_name){

         $global = getInstance();
         
         $sql = "SELECT ".$account_name." FROM " .$global->ecs->table("account") . " WHERE user_id=$user_id";
        
         $useraccount= $global->db->getRow($sql);

         return $useraccount[$account_name];
    }
    function finduserinfo($user_id,$field_name){

         $global = getInstance();
         
         $sql = "SELECT ".$field_name." FROM " .$global->ecs->table("users") . " WHERE user_id=$user_id";
        
         $users= $global->db->getRow($sql);

         return $users[$field_name];
    }
        /*会员推荐链路串接*/
    function updateuser_reposition($user_id,$parent_id=0){
        $global = getInstance();
        if($parent_id){
            
            /*有上级  上级的user_reposition+‘_’+UID  例如 上级是U2771 自己UID是2880  user_position 就是 U2771_2880*/
            $selfsql = "SELECT user_vip from " .$global->ecs->table("users") . " where user_id='$user_id'";
            $selfresult = $global->db->getRow($selfsql);
            $sql = "SELECT user_reposition,resource,user_vip from " .$global->ecs->table("users") . " where user_id='$parent_id'";
            $res = $global->db->getRow($sql);

            $parentuser_reposition = $res['user_reposition'] ;
            $resource = $res['resource'] ;
            /*如果是自己的身份是vip，则不允许变更resoure*/
            $user_reposition = $parentuser_reposition."_".$user_id;
            ...
            if(!$selfresult['user_vip']){
                ...
                */
                ...
                $sql = "UPDATE " . $global->ecs->table('users')." SET resource = '$resource',user_reposition = '$user_reposition'" .

                " WHERE user_id = '$user_id' LIMIT 1";
                $global->db->query($sql);
            }else{
               $sql = "UPDATE " . $global->ecs->table('users')." SET user_reposition = '$user_reposition'" .
...
//跟新某用户下面的所有子孩子的推荐关系
function updateAllChildReposition($user_id,$uids='',$dep=1){
    $global = getInstance();
        $userList =  $global->db->query("select user_id,parent_id from ecs_users where parent_id=$user_id");
     
        foreach ($userList as $key=>$value){
            $uids .= $value['user_id'].',';
            /*更新用户的推荐琏字段*/
            updateuser_reposition($value['user_id'],$value['parent_id']);
            
            $user = $global->db->query("SELECT user_id,parent_id from " .$global->ecs->table("users") . " where parent_id={$value['user_id']}");
            if($user){
                $uids = updateAllChildReposition($value['user_id'],$uids,$dep+1);
            }
        }
        return $uids;
    }
 ...
 * @return  mix
 */
function poster_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.poster_id) AS aricle_num ".
               ' FROM ' . $global->ecs->table('poster_cat') . " AS c".
               " LEFT JOIN " . $global->ecs->table('poster_cat') . " AS s ON s.parent_id=c.cat_id".
               " LEFT JOIN " . $global->ecs->table('poster') . " AS a ON a.cat_id=c.cat_id".
               " GROUP BY c.cat_id ".
               " ORDER BY parent_id, sort_order ASC";
            $res = $global->db->getAll($sql);
    ...
    $options = poster_cat_options($cat_id, $res); // 获得指定分类下的子分类的数组
 ...
 * @return  void
 */
function poster_cat_options($spec_cat_id, $arr)
{
    static $cat_options = array();
...
function return_action($ret_id, $return_status = 0, $refund_status = 0, $is_check = 0, $note = '', $username = null, $place = 0, $action_info = null) {
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('return_action') .
            ...
            'SELECT ' .
            "ret_id, '$username', '$return_status', '$refund_status', '$is_check', '$place', '$note', '" . time() . "' ,'" . $action_info . "' " .
 ...
 * @param string $from
 */
function add_user_message($user_id,$type,$title,$content,$from = "SYSTEM"){
    $global = getInstance();
    $message = array(
        "user_id" => $user_id,
        "type"  => $type,
    ...
    $global->db->autoExecute($global->ecs->table('user_message'), $message, 'INSERT');
}
/*获取微信用户的openid*/
...
function pushWechatTemplate($userid,$code,$pushData,$url){
    ...
    $url = urlencode(base64_encode($mobile_url.$url));
    if(file_exists($file_final) ){
        //以json格式传输
        $data = urlencode(serialize($pushData));
        $frUrl = $mobile_url . 'index.php?c=api&a=index&user_id='.$userid.'&code='.urlencode($code).'&pushData='.$data.'&url='.$url;
        curlGet($frUrl);
    }
