 ...
 */
   function givePayPoints($user_id,$goods_id){
         $global = getInstance();
         $usersql = "SELECT user_vip FROM " . $global->ecs->table('users') .
            " WHERE user_id = '$user_id'";
        $userres = $global->db->getRow($usersql);

         $goodssql = "SELECT vip_give_integral,give_integral,shop_price FROM " . $global->ecs->table('goods') .
            " WHERE goods_id = '$goods_id'";
        $goodsres = $global->db->getRow($goodssql);
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
        ...
        $order['formated_money_paid']   = price_format(abs($order['money_paid']), false);
 ...
 * @return  array   用户信息
 */
function useraccount_info($user_id)
{
    $global = getInstance();
    $sql = "SELECT * FROM " . $global->ecs->table('account') .
            " WHERE user_id = '$user_id'";
    $account = $global->db->getRow($sql);
...
//            $user['user_money'] = 0;
//        }
        $account['formated_user_money'] = price_format($account['user_money'], false);
        $account['formated_frozen_money'] = price_format($account['frozen_money'], false);
    }
 ...
 * @return  int     积分数
 */
function vip_integral_to_give($order)
{
    $global = getInstance();
    /* 判断是否团购 */
    if ($order['extension_code'] == 'group_buy')
    ...
    {
        $sql = "SELECT SUM(og.goods_number * IF(g.vip_give_integral > -1, g.vip_give_integral, og.goods_price)) AS custom_points, SUM(og.goods_number * IF(g.rank_integral > -1, g.rank_integral, og.goods_price)) AS rank_points " .
                "FROM " . $global->ecs->table('order_goods') . " AS og, " .
                          $global->ecs->table('goods') . " AS g " .
                "WHERE og.goods_id = g.goods_id " .
                "AND og.order_id = '$order[order_id]' " .
...
function aftermarket_refund($order, $refund_type, $refund_amount, $refund_note)
{
    ...
    if (1 == $refund_type) {
        ...
        new_log_account_change($user_id, $refund_amount,$change_desc,ACT_OTHER,1);
        new_log_account_change($user_id, (-1) * intval($integral['rank_points']),$change_desc,ACT_OTHER,5);
        return true;
...
function return_surplus_integral_bonus($user_id, $return_goods_price)
{
    ...
    $sql = " SELECT pay_points  FROM " . $GLOBALS['ecs']->table('account') . " WHERE user_id=" . $user_id;
    $pay = $GLOBALS['db']->getOne($sql);
    $pay = $pay - $return_goods_price; //用户总积分-本次商品

    if ($pay > 0) {
        $sql = "UPDATE " . $GLOBALS['ecs']->table('account') . " SET pay_points =" . $pay . " WHERE user_id=" . $user_id;
