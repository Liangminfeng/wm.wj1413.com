...
function upgradeToVip($user_id,$order_id){
    ...
    $sql0 = "select * from " . $this->model->pre . "order_goods  WHERE order_id=" . $order_id;
    $order_goods = $this->model->query($sql0);

    foreach ($order_goods as $key => $value) {
        $sql = "select integral,goods_number from ".$this->model->pre."goods where goods_id = ".$value['goods_id'];
       
        $res = $this->model->query($sql);
         /*查询每个商品的等级积分,0或者3000*/
        /*如果是3000,那么取这个商品的数量当作续费等级的年限*/
        if($res['0']['integral']>0){
            $num += $value['goods_number'];//多少年
     ...
     $sql1 = "select * from ". $this->model->pre."users  WHERE user_id = '$user_id'";
    ...
    if($res1['0']['vip_validated_firsttime']){
        ...
        //$t+365*24*60*60
        $sql2 = "UPDATE ". $this->model->pre."users SET rank_points = rank_points + '$rank_points', vip_validated = vip_validated + '$num' WHERE user_id = '$user_id'";
     
        $res2 = $this->model->query($sql2);

     }else{

        $sql2 = "UPDATE ". $this->model->pre."users SET rank_points = rank_points + '$rank_points', vip_validated = vip_validated + '$num',vip_validated_firsttime = '$time' WHERE user_id = '$user_id'";
