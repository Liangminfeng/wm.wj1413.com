<?php



/**

* 管理中心共用语言档

*/



$_LANG['app_name'] = ' 拓客商城管理中心';

$_LANG['cp_home'] = ' 拓客商城管理中心';

$_LANG['copyright'] = '';

$_LANG['query_info'] = ' 共执行 %d 个查询，用时 %s 秒';

$_LANG['memory_info'] = ' ，记忆体佔用 %0.3f MB';

$_LANG['gzip_enabled'] = ' ， Gzip 已启用';

$_LANG['gzip_disabled'] = ' ， Gzip 已禁用';

$_LANG['loading'] = ' 正在处理您的请求...';

$_LANG['js_languages']['process_request'] = ' 正在处理您的请求...';

$_LANG['js_languages']['todolist_caption'] = ' 记事本';

$_LANG['js_languages']['todolist_autosave'] = ' 自动储存';

$_LANG['js_languages']['todolist_save'] = ' 保存';

$_LANG['js_languages']['todolist_clear'] = ' 清除';

$_LANG['js_languages']['todolist_confirm_save'] = ' 是否将更改保存到记事本？ ';

$_LANG['js_languages']['todolist_confirm_clear'] = ' 是否清空内容？ ';

$_LANG['auto_redirection'] = ' 如果您不做出选择，将在 <span id="spanSeconds">3</span> 秒后跳转到第一个连结位址。 ';

$_LANG['password_rule'] = ' 密码应只包含英文字元、数位 . 长度在 6--16 位之间';

$_LANG['username_rule'] = ' 使用者名应为汉字、英文字元、数位组合， 3 到 15 位';

$_LANG['plugins_not_found'] = ' 外挂程式 %s 无法定位';

$_LANG['no_records'] = ' 没有找到任何记录';

$_LANG['role_describe'] = ' 角色描述';



$_LANG['require_field'] = '<span class="require-field">*</span>';

$_LANG['yes'] = ' 是';

$_LANG['no'] = ' 否';

$_LANG['record_id'] = ' 编号';

$_LANG['handler'] = ' 操作';

$_LANG['install'] = ' 安装';

$_LANG['uninstall'] = ' 卸载';

$_LANG['list'] = ' 清单';

$_LANG['add'] = ' 添加';

$_LANG['edit'] = ' 编辑';

$_LANG['view'] = ' 查看';

$_LANG['remove'] = ' 移除';

$_LANG['drop'] = ' 删除';

$_LANG['confirm_delete'] = ' 您确定要删除吗？ ';

$_LANG['disabled'] = ' 禁用';

$_LANG['enabled'] = ' 启用';

$_LANG['setup'] = ' 设置';

$_LANG['success'] = ' 成功';

$_LANG['sort_order'] = ' 排序';

$_LANG['trash'] = ' 回收站';

$_LANG['restore'] = ' 还原';

$_LANG['close_window'] = ' 关闭视窗';

$_LANG['btn_select'] = ' 选择';

$_LANG['operator'] = ' 操作人';

$_LANG['cancel'] = ' 取消';



$_LANG['empty'] = ' 不能为空';

$_LANG['repeat'] = ' 已存在';

$_LANG['is_int'] = ' 应该为整数';

$_LANG['talk_title'] = ' 话题标题';

$_LANG['button_submit'] = ' 确定 ';
$_LANG['succed'] = ' 成功 ';

$_LANG['button_save'] = ' 保存 ';

$_LANG['button_reset'] = ' 重置 ';

$_LANG['button_search'] = ' 搜索 ';
$_LANG['talk_user_name'] = '用户名称'; 
$_LANG['sucai_add'] = '添加素材';

$_LANG['priv_error'] = ' 对不起 , 您没有执行此项操作的许可权!';

$_LANG['drop_confirm'] = ' 您确认要删除这条记录吗?';

$_LANG['form_notice'] = ' 点击此处查看提示资讯';

$_LANG['upfile_type_error'] = ' 上传档的类型不正确!';

$_LANG['upfile_error'] = ' 上传档失败!';

$_LANG['no_operation'] = ' 您没有选择任何操作';



$_LANG['go_back'] = ' 返回上一页';

$_LANG['back'] = ' 返回';

$_LANG['continue'] = ' 继续';

$_LANG['system_message'] = ' 系统资讯';

$_LANG['check_all'] = ' 全选';

$_LANG['select_please'] = ' 请选择...';

$_LANG['all_category'] = ' 所有分类';

$_LANG['all_brand'] = ' 所有品牌';

$_LANG['refresh'] = ' 刷新';

$_LANG['update_sort'] = ' 更新排序';

$_LANG['modify_failure'] = ' 修改失败!';

$_LANG['attradd_succed'] = ' 操作成功!';

$_LANG['todolist'] = ' 记事本';

$_LANG['n_a'] = 'N/A';



/* 提示 */

$_LANG['sys']['wrong'] = ' 错误：';



/* 编码 */

$_LANG['charset']['utf8'] = ' 国际化编码（ utf8 ）';

$_LANG['charset']['zh_cn'] = ' 简体中文';

$_LANG['charset']['zh_tw'] = ' 繁体中文';

$_LANG['charset']['en_us'] = ' 美国英语';

$_LANG['charset']['en_uk'] = ' 英文';



/* 新订单通知 */

$_LANG['order_notify'] = ' 新订单通知';

$_LANG['new_order_1'] = ' 您有 ';

$_LANG['new_order_2'] = ' 个新订单以及 ';

$_LANG['new_order_3'] = ' 个新付款的订单';

$_LANG['new_order_link'] = ' 点击查看新订单';



/*语言项*/

$_LANG['chinese_simplified'] = ' 简体中文';

$_LANG['english'] = ' 英文';



/* 分页 */

$_LANG['total_records'] = ' 总计 ';

$_LANG['total_pages'] = ' 个记录分为';

$_LANG['page_size'] = ' 页，每页';

$_LANG['page_current'] = ' 页当前第';

$_LANG['page_first'] = ' 第一页';

$_LANG['page_prev'] = ' 上一页';

$_LANG['page_next'] = ' 下一页';

$_LANG['page_last'] = ' 最末页';

$_LANG['admin_home'] = ' 起始页';



/* 重量 */

$_LANG['gram'] = ' 克';

$_LANG['kilogram'] = ' 千克';



/* 功能表分类部分 */

$_LANG['02_cat_and_goods'] = ' 商品管理';

$_LANG['03_promotion'] = ' 促销管理';

$_LANG['04_order'] = ' 订单管理';

$_LANG['05_banner'] = ' 广告管理';

$_LANG['06_stats'] = ' 报表统计';

$_LANG['07_content'] = ' 文章管理';

$_LANG['08_members'] = ' 会员管理';

$_LANG['09_others'] = ' 杂项管理';

$_LANG['10_priv_admin'] = ' 版权管理';

$_LANG['11_system'] = ' 系统设置';

$_LANG['12_template'] = ' 范本管理';

$_LANG['13_backup'] = ' 资料库管理';

$_LANG['14_sms'] = ' 短信管理';

$_LANG['15_rec'] = ' 推荐管理';

$_LANG['16_email_manage'] = ' 邮件群发管理';



/* 商品管理 */

$_LANG['01_goods_list'] = ' 商品清单';

$_LANG['02_goods_add'] = ' 添加新商品';

$_LANG['03_category_list'] = ' 商品分类';

$_LANG['04_category_add'] = ' 添加分类';

$_LANG['05_comment_manage'] = ' 使用者评论';

$_LANG['06_goods_brand_list'] = ' 商品品牌';

$_LANG['06_goods_kd_cash_scale'] = 'KD豆现金使用比例';
$_LANG['66_partner_manage'] = '供应商接口管理';
$_LANG['07_brand_add'] = ' 添加品牌';

$_LANG['08_goods_type'] = ' 商品类型';

$_LANG['09_attribute_list'] = ' 商品属性';

$_LANG['10_attribute_add'] = ' 添加属性';

$_LANG['11_goods_trash'] = ' 商品回收站';

$_LANG['12_batch_pic'] = ' 图片批量处理';

$_LANG['13_batch_add'] = ' 商品批量上传';

$_LANG['15_batch_edit'] = ' 商品批量修改';

$_LANG['16_goods_script'] = ' 生成商品代码';

$_LANG['17_tag_manage'] = ' 标籤管理';

$_LANG['18_product_list'] = ' 货品清单';

$_LANG['52_attribute_add'] = ' 编辑属性';

$_LANG['53_suppliers_goods'] = ' 供应商商品管理';



$_LANG['14_goods_export'] = ' 商品批量汇出';



$_LANG['50_virtual_card_list'] = ' 虚拟商品清单';

$_LANG['51_virtual_card_add'] = ' 添加虚拟商品';

$_LANG['52_virtual_card_change'] = ' 更改加密串';

$_LANG['goods_auto'] = ' 商品自动上下架';

$_LANG['article_auto'] = ' 文章自动发佈';

$_LANG['navigator'] = $_LANG['01_navigator'] = ' 导航设置';



/* 促销管理 */

$_LANG['02_snatch_list'] = ' 夺宝奇兵';

$_LANG['snatch_add'] = ' 添加夺宝奇兵';

$_LANG['04_bonustype_list'] = ' 红包类型';

$_LANG['bonustype_add'] = ' 添加红包类型';

$_LANG['05_bonus_list'] = ' 线下红包';

$_LANG['bonus_add'] = ' 添加会员红包';

$_LANG['06_pack_list'] = ' 商品包装';

$_LANG['07_card_list'] = ' 祝福贺卡';

$_LANG['pack_add'] = ' 添加新包装';

$_LANG['card_add'] = ' 添加新贺卡';

$_LANG['08_group_buy'] = ' 团购活动';

$_LANG['09_topic'] = ' 专题管理';

$_LANG['topic_add'] = ' 添加专题';

$_LANG['talk_add'] = ' 添加话题';

$_LANG['topic_list'] = ' 专题清单';

$_LANG['10_auction'] = ' 拍卖活动';

$_LANG['12_favourable'] = ' 优惠活动';

$_LANG['13_wholesale'] = ' 批发管理';

$_LANG['ebao_commend'] = ' 易宝推荐';

$_LANG['14_package_list'] = ' 超值礼包';

$_LANG['package_add'] = ' 添加超值礼包';

$_LANG['16_train'] = ' 超级列车';
$_LANG['17_lottery'] = ' 鱼票抽奖';
$_LANG['18_talk'] = '话题管理';
$_LANG['19_sucai'] = '动态素材';

/* 订单管理 */

$_LANG['02_order_list'] = ' 订单清单';

$_LANG['03_order_query'] = ' 订单查询';

$_LANG['04_merge_order'] = ' 合併订单';

$_LANG['05_edit_order_print'] = ' 订单列印';

$_LANG['06_undispose_booking'] = ' 缺货登记';

$_LANG['08_add_order'] = ' 添加订单';

$_LANG['09_delivery_order'] = ' 发货单清单';

$_LANG['10_back_order'] = ' 退货单清单';

/* by ECTouch leah begin*/

$_LANG['13_service_type'] = ' 售后服务类型';

$_LANG['02_add_service_type'] = ' 添加售后服务类型';

$_LANG['04_aftermarket'] = ' 售后服务';

$_LANG['10_cause_add'] = ' 添加退换货原因';

$_LANG['11_back_cause'] = ' 退换货原因清单';

$_LANG['12_back_apply'] = ' 退换货清单';

/* by Leah end*/



/* 广告管理 */

$_LANG['ad_position'] = ' 广告位置';

$_LANG['ad_list'] = ' 广告清单';



/* 报表统计 */

$_LANG['flow_stats'] = ' 流量分析';

$_LANG['searchengine_stats'] = ' 搜寻引擎';

$_LANG['report_order'] = ' 订单统计';

$_LANG['report_sell'] = ' 销售概况';

$_LANG['sell_stats'] = ' 销售排行';

$_LANG['sale_list'] = ' 销售明细';

$_LANG['report_guest'] = ' 客户统计';

$_LANG['report_users'] = ' 会员排行';

$_LANG['visit_buy_per'] = ' 访问购买率';

$_LANG['z_clicks_stats'] = ' 站外投放 JS';



/* 文章管理 */

$_LANG['02_articlecat_list'] = ' 文章分类';


$_LANG['articlecat_add'] = ' 添加文章分类';


$_LANG['03_article_list'] = ' 文章清单';


$_LANG['article_add'] = ' 添加新文章';


$_LANG['shop_article'] = ' 网店文章';

$_LANG['shop_info'] = ' 网店资讯';

$_LANG['shop_help'] = ' 网店说明';

$_LANG['vote_list'] = ' 线上调查';

// 海报管理
$_LANG['32_postercat_list'] = ' 海报分类';
$_LANG['postercat_add'] = ' 添加海报分类';
$_LANG['33_poster_list'] = ' 海报清单';
$_LANG['poster_add'] = ' 添加新海报';

/* 会员管理 */

$_LANG['08_unreply_msg'] = ' 会员留言';

$_LANG['03_users_list'] = ' 会员清单';

$_LANG['04_accountlog_list'] = ' 资金流水';

$_LANG['04_users_add'] = ' 添加会员';
$_LANG['13_users_add'] = '手动补单';

$_LANG['05_user_rank_list'] = ' 会员等级';

$_LANG['06_list_integrate'] = ' 会员整合';

$_LANG['09_user_account'] = ' 充值和提现申请';

$_LANG['10_user_account_manage'] = ' 资金管理';

$_LANG['11_users_account_log'] = ' 会员积分记录';

$_LANG['12_user_autonym'] = ' 实名认证申请';


/* 版权管理 */

$_LANG['admin_list'] = ' 管理员清单';

$_LANG['admin_list_role'] = ' 角色清单';

$_LANG['admin_role'] = ' 角色管理';

$_LANG['admin_add'] = ' 添加管理员';

$_LANG['admin_add_role'] = ' 添加角色';

$_LANG['admin_edit_role'] = ' 修改角色';

$_LANG['admin_logs'] = ' 管理员日志';

$_LANG['agency_list'] = ' 办事处清单';

$_LANG['suppliers_list'] = ' 供应商清单';



/* 系统设置 */

$_LANG['01_shop_config'] = ' 商店设置';

$_LANG['shop_authorized'] = ' 授权证书';

$_LANG['shp_webcollect'] = ' 网罗天下';

$_LANG['02_payment_list'] = ' 支付方式';

$_LANG['03_shipping_list'] = ' 配送方式';
$_LANG['04_goods_transport'] = ' 运费管理';
$_LANG['04_mail_settings'] = $_LANG['03_mail_settings'] = ' 邮件伺服器设置';

$_LANG['05_area_list'] = ' 地区清单';

$_LANG['07_cron_schcron'] = ' 计画任务';

$_LANG['08_friendlink_list'] = ' 友情连结';



$_LANG['shipping_area_list'] = ' 配送区域';

$_LANG['sitemap'] = ' 网站地图';

$_LANG['check_file_priv'] = ' 档许可权检测';

$_LANG['captcha_manage'] = $_LANG['05_captcha_manage'] = ' 验证码管理';

$_LANG['fckfile_manage'] = 'Fck 上传档管理';

$_LANG['ucenter_setup'] = 'UCenter 设置';

$_LANG['file_check'] = ' 档校验';

$_LANG['021_reg_fields'] = ' 会员注册项设置';



/* 范本管理 */

$_LANG['02_template_select'] = ' 范本选择';

$_LANG['03_template_setup'] = ' 设置范本';

$_LANG['04_template_library'] = ' 库专案管理';

$_LANG['mail_template_manage'] = ' 邮件范本';

$_LANG['05_edit_languages'] = ' 语言项编辑';

$_LANG['06_template_backup'] = ' 范本设置备份';

/* 资料库管理 */

$_LANG['02_db_manage'] = ' 资料备份';

$_LANG['03_db_optimize'] = ' 资料表优化';

$_LANG['04_sql_query'] = 'SQL 查询';

$_LANG['05_synchronous'] = ' 同步资料';

$_LANG['convert'] = ' 转换资料';



/* 短信管理 */

$_LANG['02_sms_my_info'] = ' 帐号资讯';

$_LANG['03_sms_send'] = ' 传送简讯';

$_LANG['04_sms_charge'] = ' 帐户充值';

$_LANG['05_sms_send_history'] = ' 发送记录';

$_LANG['06_sms_charge_history'] = ' 充值记录';







$_LANG['05_affiliate'] = ' 分成佣金设置';

$_LANG['06_affiliate_ck'] = ' 分成订单管理';

$_LANG['affiliate'] = ' 推荐设置';

$_LANG['affiliate_ck'] = ' 分成管理';

$_LANG['flashplay'] = ' 首页主广告管理';

$_LANG['search_log'] = ' 搜索关键字';

$_LANG['email_list'] = ' 邮件订阅管理';

$_LANG['magazine_list'] = ' 杂志管理';

$_LANG['attention_list'] = ' 关注管理';

$_LANG['view_sendlist'] = $_LANG['04_view_sendlist'] = ' 邮件伫列管理';



/* 积分兑换管理 */

$_LANG['15_exchange_goods'] = ' 积分商城商品';

$_LANG['15_exchange_goods_list'] = ' 积分商城商品清单';

$_LANG['exchange_goods_add'] = ' 添加新商品';



/* cls_image 类的语言项 */

$_LANG['directory_readonly'] = ' 目录 % 不存在或不可写';

$_LANG['invalid_upload_image_type'] = ' 不是允许的图片格式';

$_LANG['upload_failure'] = ' 档 %s 上传失败。 ';

$_LANG['missing_gd'] = ' 没有安装 GD 库';

$_LANG['missing_orgin_image'] = ' 找不到原始图片 %s ';

$_LANG['nonsupport_type'] = ' 不支援该图像格式 %s ';

$_LANG['creating_failure'] = ' 创建图片失败';

$_LANG['writting_failure'] = ' 图片写入失败';

$_LANG['empty_watermark'] = ' 浮水印档参数不能为空';

$_LANG['missing_watermark'] = ' 找不到浮水印档 %s';

$_LANG['create_watermark_res'] = ' 创建浮水印图片资源失败。 浮水印图片类型为%s';

$_LANG['create_origin_image_res'] = ' 创建原始图片资源失败，原始图片类型 %s';

$_LANG['invalid_image_type'] = ' 无法识别浮水印图片 %s ';

$_LANG['file_unavailable'] = ' 档 %s 不存在或不可读';



/* 邮件发送错误资讯 */

$_LANG['smtp_setting_error'] = ' 邮件伺服器设置资讯不完整';

$_LANG['smtp_connect_failure'] = ' 无法连接到邮件伺服器 %s';

$_LANG['smtp_login_failure'] = ' 邮件伺服器验证帐号或密码不正确';

$_LANG['sendemail_false'] = ' 邮件发送失败，请检查您的邮件伺服器设置！ ';

$_LANG['smtp_refuse'] = ' 伺服器拒绝发送该邮件';

$_LANG['disabled_fsockopen'] = ' 伺服器已禁用 fsocketopen 函数。 ';



/* 虚拟卡 */

$_LANG['virtual_card_oos'] = ' 虚拟卡已缺货';



$_LANG['span_edit_help'] = ' 点击修改内容';

$_LANG['href_sort_help'] = ' 点击对清单排序';



$_LANG['catname_exist'] = ' 已存在相同的分类名称!';

$_LANG['brand_name_exist'] = ' 已存在相同的品牌名称!';



$_LANG['alipay_login'] = '<a href="HTTPs://www.alipay.com/user/login.htm?goto=HTTPs%3A%2F%2Fwww.alipay.com% 2Fhimalayas%2Fpracticality_profile_edit.htm%3Fmarket_type%3Dfrom_agent_contract%26customer_external_id%3D% 2BC4335319945672464113" target="_blank">立即免费申请支付介面许可权</a>';

$_LANG['alipay_look'] = '<a href=\"HTTPs://www.alipay.com/himalayas/practicality.htm\" target=\"_blank\"> 请申请成功后登录支付宝帐户查看 </a>';







$_LANG['04_sms_sign'] = ' 短信签名';





/* 后台管理功能表 */

$_LANG['00_menu_dashboard'] = ' 主控台';

$_LANG['01_menu_system'] = ' 设置';

$_LANG['02_menu_goods'] = ' 商品';

$_LANG['03_menu_order'] = ' 订单';

$_LANG['031_menu_aftermarket'] = ' 售后服务';

$_LANG['04_menu_members'] = ' 会员';

$_LANG['05_menu_content'] = ' 内容';

$_LANG['06_menu_promotion'] = ' 行销';

$_LANG['07_menu_stats'] = ' 报表';

$_LANG['08_menu_template'] = ' 范本';

$_LANG['09_menu_backup'] = ' 资料库';

$_LANG['10_menu_tools'] = ' 工具箱';



$_LANG['12_menu_wechat'] = ' 微信通';



// 主控台

$_LANG['01_dashboard_welcome'] = ' 欢迎页面';

//$_LANG['02_favorite'] = ' 常用功能';

//$_LANG['03_notepad'] = ' 记事本';

//$_LANG['04_calc'] = ' 计算机';



// 设置

$_LANG['01_shop_config'] = ' 商店设置';

$_LANG['02_payment_list'] = ' 支付方式';

$_LANG['03_shipping_list'] = ' 配送方式';

$_LANG['04_area_list'] = ' 地区清单';

$_LANG['05_admin_list'] = ' 管理员清单';

$_LANG['06_admin_role'] = ' 角色管理';

$_LANG['07_admin_logs'] = ' 管理员日志';

$_LANG['08_agency_list'] = ' 办事处清单';

$_LANG['09_suppliers_list'] = ' 供应商清单';

$_LANG['10_shop_authorized'] = ' 授权证书';



// 商品





// 订单





// 会员





// 内容





// 行销





// 报表





// 范本





// 资料库



// 工具箱

$_LANG['01_navigator'] = ' 导航设置';

$_LANG['02_authorization'] = ' 登录外挂程式';

$_LANG['03_mail_settings'] = ' 邮件伺服器设置';

$_LANG['04_view_sendlist'] = ' 邮件伫列管理';

$_LANG['05_captcha_manage'] = ' 验证码管理';

$_LANG['99_upgrade'] = ' 线上更新';







// 微信通

$_LANG['01_wechat_config'] = ' 公众号设置';

$_LANG['02_wechat_masssend'] = ' 群发消息';

$_LANG['03_wechat_autoreply'] = ' 自动回复';

$_LANG['04_wechat_selfmenu'] = ' 自订功能表';

$_LANG['05_wechat_tmplmsg'] = ' 范本消息';

$_LANG['06_wechat_contactmanage'] = ' 使用者管理';

$_LANG['07_wechat_appmsg'] = ' 素材管理';

$_LANG['08_wechat_qrcode'] = ' 扫码引荐';

$_LANG['09_wechat_extends'] = ' 扩展功能';

// $_LANG['10_wechat_remind'] = ' 提醒设置';

$_LANG['11_wechat_customer'] = ' 多客服设置';

// $_LANG['12_wechat_wall'] = ' 微信牆';


