<?php
/**
 * ECSHOP 短信模組語言檔
 * ============================================================================
 * * 版權所有 2005-2012 上海商派網路科技有限公司，並保留擁有權利。
 * 網站位址 : HTTP://www.ecshop.com ；
 * ----------------------------------------------------------------------------
 * 這不是一個自由軟體！ 您只能在不用於商業目的的前提下對程式碼進行修改和
 * 使用；不允許對程式碼以任何形式任何目的的再發佈。
 * ============================================================================
 * $Author: liubo $
 * $Id: sms.php 17217 2011-01-19 06:29:08Z liubo $
 */

/* 導航條 */
$_LANG['register_sms'] = ' 註冊或啟用短信帳號';

/* 註冊和啟用短信功能 */
$_LANG['email'] = ' 電子郵箱';
$_LANG['password'] = ' 登錄密碼';
$_LANG['domain'] = ' 網店功能變數名稱';
$_LANG['register_new'] = ' 註冊新帳號';
$_LANG['error_tips'] = ' 請在商店設置 -> 短信設置，先註冊短信服務並正確配置短信服務！ ';
$_LANG['enable_old'] = ' 啟用已有帳號';

/* 短信特服資訊 */
$_LANG['sms_user_name'] = ' 使用者名：';
$_LANG['sms_password'] = ' 密碼：';
$_LANG['sms_domain'] = ' 功能變數名稱：';
$_LANG['sms_num'] = ' 短信特服號：';
$_LANG['sms_count'] = ' 傳送簡訊條數：';
$_LANG['sms_total_money'] = ' 總共沖值金額：';
$_LANG['sms_balance'] = ' 餘額：';
$_LANG['sms_last_request'] = ' 最後一次請求時間：';
$_LANG['disable'] = ' 登出短信服務';

/* 傳送簡訊 */
$_LANG['phone'] = ' 接收手機號碼';
$_LANG['user_rand'] = ' 按使用者等級發送短消息';
$_LANG['phone_notice'] = ' 多個手機號碼用半形逗號分開';
$_LANG['msg'] = ' 消息內容';
$_LANG['msg_notice'] = ' 最長 67 字元，簽名已經預設，不需要添加簽名。 （如未設置簽名點擊 <a href="sms.php?act=sms_sign">設置</a> ）';
$_LANG['send_date'] = ' 定時發送時間';
$_LANG['send_date_notice'] = ' 格式為 YYYY-MM-DD HH:II 。 為空表示立即發送。 ';
$_LANG['back_send_history'] = ' 返回發送歷史清單';
$_LANG['back_charge_history'] = ' 返回充值歷史清單';

/* 記錄查詢介面 */
$_LANG['start_date'] = ' 開始日期';
$_LANG['date_notice'] = ' 格式為 YYYY-MM-DD 。 可為空。 ';
$_LANG['end_date'] = ' 結束日期';
$_LANG['page_size'] = ' 每頁顯示數量';
$_LANG['page_size_notice'] = ' 可為空，表示每頁顯示 20 條記錄';
$_LANG['page'] = ' 頁數';
$_LANG['page_notice'] = ' 可為空，表示顯示 1 頁';
$_LANG['charge'] = ' 請輸入您想要充值的金額';

/* 動作確認資訊 */
$_LANG['history_query_error'] = ' 對不起，在查詢過程中發生錯誤。 ';
$_LANG['enable_ok'] = ' 恭喜，您已成功啟用短信服務！ ';
$_LANG['enable_error'] = ' 對不起，您啟用短信服務失敗。 ';
$_LANG['disable_ok'] = ' 您已經成功登出短信服務。 ';
$_LANG['disable_error'] = ' 登出短信服務失敗。 ';
$_LANG['register_ok'] = ' 恭喜，您已成功註冊短信服務！ ';
$_LANG['register_error'] = ' 對不起，您註冊短信服務失敗。 ';
$_LANG['send_ok'] = ' 恭喜，您的短信已經成功發送！ ';
$_LANG['send_error'] = ' 對不起，在傳送簡訊過程中發生錯誤。 ';
$_LANG['error_no'] = ' 錯誤標識';
$_LANG['error_msg'] = ' 錯誤描述';
$_LANG['empty_info'] = ' 您的短信特服資訊為空。 ';

/* 充值記錄 */
$_LANG['order_id'] = ' 訂單號';
$_LANG['money'] = ' 充值金額';
$_LANG['log_date'] = ' 充值日期';

/* 發送記錄 */
$_LANG['sent_phones'] = ' 發送手機號碼';
$_LANG['content'] = ' 發送內容';
$_LANG['charge_num'] = ' 計費條數';
$_LANG['sent_date'] = ' 發送日期';
$_LANG['send_status'] = ' 發送狀態';
$_LANG['status'][0] = ' 失敗';
$_LANG['status'][1] = ' 成功';
$_LANG['user_list'] = ' 全體會員';
$_LANG['please_select'] = ' 請選擇會員等級';

/* 提示 */
$_LANG['test_now'] = '<span style="color:red;" ></span>';
$_LANG['msg_price'] = '<span style="color:green;" >短信每條0.1元(RMB)</span>';

/* API 返回的錯誤資訊 */
//--註冊
$_LANG['api_errors']['register'][1] = ' 功能變數名稱不能為空。 ';
$_LANG['api_errors']['register'][2] = ' 郵箱填寫不正確。 ';
$_LANG['api_errors']['register'][3] = ' 使用者名已存在。 ';
$_LANG['api_errors']['register'][4] = ' 未知錯誤。 ';
$_LANG['api_errors']['register'][5] = ' 介面錯誤。 ';
//--獲取餘額
$_LANG['api_errors']['get_balance'][1] = ' 使用者名密碼不正確。 ';
$_LANG['api_errors']['get_balance'][2] = ' 使用者被禁用。 ';
//--傳送簡訊
$_LANG['api_errors']['send'][1] = ' 使用者名密碼不正確。 ';
$_LANG['api_errors']['send'][2] = ' 短信內容過長。 ';
$_LANG['api_errors']['send'][3] = ' 發送日期應大於目前時間。 ';
$_LANG['api_errors']['send'][4] = ' 錯誤的號碼。 ';
$_LANG['api_errors']['send'][5] = ' 帳戶餘額不足。 ';
$_LANG['api_errors']['send'][6] = ' 帳戶已被停用。 ';
$_LANG['api_errors']['send'][7] = ' 介面錯誤。 ';
//--歷史記錄
$_LANG['api_errors']['get_history'][1] = ' 使用者名密碼不正確。 ';
$_LANG['api_errors']['get_history'][2] = ' 查無記錄。 ';
//--使用者驗證
$_LANG['api_errors']['auth'][1] = ' 密碼錯誤。 ';
$_LANG['api_errors']['auth'][2] = ' 使用者不存在。 ';

/* 使用者伺服器檢測到的錯誤資訊 */
$_LANG['server_errors'][1] = ' 註冊資訊無效。 '; //ERROR_INVALID_REGISTER_INFO
$_LANG['server_errors'][2] = ' 啟用資訊無效。 '; //ERROR_INVALID_ENABLE_INFO
$_LANG['server_errors'][3] = ' 發送的資訊有誤。 '; //ERROR_INVALID_SEND_INFO
$_LANG['server_errors'][4] = ' 填寫的查詢資訊有誤。 '; //ERROR_INVALID_HISTORY_QUERY
$_LANG['server_errors'][5] = ' 不正確身份資訊。 '; //ERROR_INVALID_PASSPORT
$_LANG['server_errors'][6] = 'URL 不對。 '; //ERROR_INVALID_URL
$_LANG['server_errors'][7] = 'HTTP 回應體為空。 '; //ERROR_EMPTY_RESPONSE
$_LANG['server_errors'][8] = ' 不正確 XML 檔。 '; //ERROR_INVALID_XML_FILE
$_LANG['server_errors'][9] = ' 不正確節點名字。 '; //ERROR_INVALID_NODE_NAME
$_LANG['server_errors'][10] = ' 存儲失敗。 '; //ERROR_CANT_STORE
$_LANG['server_errors'][11] = ' 短信功能尚未啟動。 '; //ERROR_INVALID_PASSPORT

/* 用戶端 JS 語言項 */
//--註冊或啟用
$_LANG['js_languages']['password_empty_error'] = ' 密碼不能為空。 ';
$_LANG['js_languages']['username_empty_error'] = ' 使用者名不能為空。 ';
$_LANG['js_languages']['username_format_error'] = ' 使用者名格式不對。 ';
$_LANG['js_languages']['domain_empty_error'] = ' 功能變數名稱不能為空。 ';
$_LANG['js_languages']['domain_format_error'] = ' 功能變數名稱格式不對。 ';
$_LANG['js_languages']['send_empty_error'] = ' 發送手機號與發送等級至少填寫一項！ ';
//--發送
$_LANG['js_languages']['phone_empty_error'] = ' 請填寫手機號。 ';
$_LANG['js_languages']['phone_format_error'] = ' 手機號碼格式不對。 ';
$_LANG['js_languages']['msg_empty_error'] = ' 請填寫消息內容。 ';
$_LANG['js_languages']['send_date_format_error'] = ' 定時發送時間格式不對。 ';
//--歷史記錄
$_LANG['js_languages']['start_date_format_error'] = ' 開始日期格式不對。 ';
$_LANG['js_languages']['end_date_format_error'] = ' 結束日期格式不對。 ';
//--充值
$_LANG['js_languages']['money_empty_error'] = ' 請輸入您要充值的金額。 ';
$_LANG['js_languages']['money_format_error'] = ' 金額格式不對。 ';



$_LANG['insert_sign'] = ' 請輸入短信簽名';
$_LANG['insert_succ'] = ' 添加成功';
$_LANG['error_smg'] = ' 資訊錯誤';
$_LANG['edit_succ'] = ' 修改成功';
$_LANG['default_succ'] = ' 簽名預設成功';
$_LANG['no_default'] = ' 未有此預設值';
$_LANG['add_sign'] = ' 新添短信簽名';
$_LANG['new_default_sign'] = ' 當前預設簽名為';
$_LANG['default_sign'] = ' 原簽名';
$_LANG['add'] = ' 添加';
$_LANG['edited'] = ' 修改為';
$_LANG['edit'] = ' 修改';
$_LANG['set_default_sign'] = ' 設置成預設簽名';


?>