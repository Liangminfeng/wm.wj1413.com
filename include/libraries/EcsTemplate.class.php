...
//            }
            ...
            switch ($tag_sel) {
                ...
                case 'format':
                    ...
                    $t= $this->get_format(substr($tag, 6));
                    return $t;
                    break;
                case 'dateformat':
                    $t= $this->get_dateFormat(substr($tag, 10));
                    return $t;
                    break;
                default:
                    return '{' . $tag . '}';
    ...
    function get_dateFormat($tag_args){
        
        $start = strpos($tag_args, '"')+1;
        $length = strrpos($tag_args, '"')-$start;
        $formatStr = substr($tag_args, $start,$length);
        $g = strpos($tag_args, ",");
        $date = trim(substr($tag_args,0,strpos($tag_args, ",")));
        $token = $this->get_val(substr($date,1));
        return "<?php echo dateformat('{$formatStr}',{$token}); ?>";
    }
    
    function get_format($tag_args){
        $tag_args = str_replace(" ", "", $tag_args);

        

        list($val_args,$decimals,$decimalpoint,$separator) =explode( ",",$tag_args);
        
        $token = $this->get_val(substr($val_args, 1));
        ...
        return "<?php echo number_format($token,'$decimals','$decimalpoint','$separator'); ?>";
    }
    function get_math_para($tag_args){
