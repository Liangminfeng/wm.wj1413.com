...
class wxpay
{
    ...
    public function get_code($order, $payment)
    {
        ...
        $options = array(
             ...
             'mch_id' => "1532586731", //微信支付商户号
             'key' => "qQa2pOWT94FsNmpCWBY7bQnLlPvMVMZ4" //微信支付API密钥
        );
        ...
        $outTradeNo = $order['order_sn'] . 'AJ' . $order_amount . 'B'.$order['id'];
        ...
        if(!is_wechat_browser()){
            ...
            $this->setParameter("out_trade_no", $order['order_sn'] . 'AJ' . $order_amount . 'B'.$order['id']); // 商户订单号
            $this->setParameter("total_fee", $order_amount); // 总金额
            $this->setParameter("spbill_create_ip", $this->get_client_ip()); // 客户端IP
            $this->setParameter("notify_url", __URL__."/join/pay_notify"); // 异步通知地址
            ...
            if(isset($respond['mweb_url'])){
                if ($respond['result_code'] == 'SUCCESS') {
                    $redirect_url = __URL__ . "/join/done?id=".$order['id'];
                }
            ...
            $returnUrl =  __URL__."/join/done";
            // wxjsbridge
            $js = '<script language="javascript">
            function jsApiCall(){WeixinJSBridge.invoke("getBrandWCPayRequest",' . $jsApiParameters . ',function(res){if(res.err_msg == "get_brand_wcpay_request:ok"){location.href="' . __URL__ . '/join/done?status=1&id='.$order['id'].'"}else{location.href="'.$returnUrl.'?status=0&id='.$order['id'].'"}})};function callpay(){if (typeof WeixinJSBridge == "undefined"){if( document.addEventListener ){document.addEventListener("WeixinJSBridgeReady", jsApiCall, false);}else if (document.attachEvent){document.attachEvent("WeixinJSBridgeReady", jsApiCall);document.attachEvent("onWeixinJSBridgeReady", jsApiCall);}}else{jsApiCall();}}
            </script>';
           

            $button = '<div style="text-align:center">
            <button class="btn btn-info ect-btn-info ect-colorf ect-bgLike" type="button" onclick="callpay()">立即付款</button>
            </div>' . $js;
    ...
    public function notify()
    {
        ...
        $inputdata = file_get_contents("php://input");
         $con = json_encode($inputdata).date("Y-m-d H:i:s");
     
         //file_put_contents(ROOT_PATH."wxtest1.log", $con, FILE_APPEND);
        ...
        if (!empty($inputdata)) {
            ...
            if ($wxsign == $sign) {
                ...
                if ($postdata['result_code'] == 'SUCCESS') {
                    ...
                    model('Base')->model->table('join_us')
                        ->data('pay_status = 1')
                        ->where('id = ' . $log_id)
                        ->update();
                }
