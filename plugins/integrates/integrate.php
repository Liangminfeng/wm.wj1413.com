...
class integrate
{
    ...
    {
        if ($this->new_check_user($username, $password) > 0) {
    ...
    function distributionLogin($username,$password,$resource,$remember = null){
        if ($this->distributionCheckUser($username, $password,$resource) > 0) {
    ...
    /*经销商登录*/
    function newdistributionLogin($account,$password,$resource,$remember = null){
        if ($this->newdistributionCheckUser($account, $password,$resource,$remember = null) > 0) {
    ...
    function distributionCheckUser($username,$password = null,$resource){
        ...
        if ($password === null) {
            ...
            ->field($this->field_id)
            ->where('user_name' . "='" . $post_username . "' and resource = ".$resource)
            ->getOne();
        } else {
            
            
            $res =  $this->db->table($this->user_table)
            ->field($this->field_pass)
            ->where('user_name' . "='" . $post_username ."' and resource = ".$resource )
            ->getOne();
            $result =  password_verify($password,$res);
            if($result){
                return 1;
            }else{
                return 0;
    ...
    function newdistributionCheckUser($account,$password = null,$resource,$remember){
       
        /* 如果没有定义密码则只检查用户名 */
        /*查询vip_manage_account*/
        $data['vip_manage_account'] = $account;
        $r = $this->db->table("vip_account")
                ->field("user_id")
                ->where($data)
                ->getOne();

        if(!$r){
            /*vip 账号不存在，不是经销商或者店主，不允许登录*/
            return false;
            ...
            $r1 = $this->db->table("users")
                ->field("status,user_name")
                ->where($data1)
                ->find();
        ...
        if ($password === null) {
            ...
            if($result){
                ...
                $this->set_session($post_username);
            
                $this->set_cookie($post_username, $remember);
                return 1;
            }else{
                return 0;
            }
    ...
    function new_check_user($username,$password = null){
        $post_username = $username;
        /* 如果没有定义密码则只检查用户名 */
        if ($password === null) {
            return $this->db->table($this->user_table)
    ...
    function add_user($username, $password, $email, $gender = -1, $bday = 0, $reg_date = 0, $md5password = '')
    {
        ...
        if ($md5password) {
            ...
            $post_password = $this->new_compile_password(array(
    ...
    function edit_user($cfg)
    {
        ...
        if ((! empty($cfg['md5password'])) && $this->field_pass != 'NULL') {
            $values[] = $this->field_pass . "='" . $this->new_compile_password(array(
    ...
    function check_user($username, $password = null)
    {
        ...
        if ($password === null) {
            ...
            return $this->db->table($this->user_table)
                ->field($this->field_id)
                ->where($this->field_name . "='" . $post_username . "' AND " . $this->field_pass . " ='" . $this->new_compile_password(array(
                'password' => $password
            )) . "'")
                ->getOne();
        }
    ...
    //         $password = password_hash($cfg['password'], 1,['cost'=>10]);
    ...
    function set_session($username = '')
    {
        if (empty($username)) {
            ...
            $sql = "SELECT user_id, password,resource, email,nick_name,rank_points,openid FROM " . $this->db->pre . 'users ' . " WHERE user_name='$username' LIMIT 1";
            ...
            if ($row) {
                ...
                // $sql = "SELECT * FROM " . $this->db->pre . 'user_rank ' . " WHERE rank_id=2";
    ...
    function sync($username, $password = '', $md5password = '')
    {
        ...
        if (empty($profile)) {
            ...
            if (empty($md5password)) {
                $sql = "INSERT INTO " . $this->db->pre . 'users ' . "(user_name, email, sex, birthday, reg_time,user_vip)" . " VALUES('$username', '" . $main_profile['email'] . "','" . $main_profile['sex'] . "','" . $main_profile['birthday'] . "','" . $main_profile['reg_time'] . "',1)";
            } else {
                $sql = "INSERT INTO " . $this->db->pre . 'users ' . "(user_name, email, sex, birthday, reg_time, password,user_vip)" . " VALUES('$username', '" . $main_profile['email'] . "','" . $main_profile['sex'] . "','" . $main_profile['birthday'] . "','" . $main_profile['reg_time'] . "', '$md5password',1)";
            }
